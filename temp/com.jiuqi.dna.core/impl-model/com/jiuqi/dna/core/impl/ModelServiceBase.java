...
import com.jiuqi.dna.core.def.model.ModelActionDefine;
import com.jiuqi.dna.core.def.model.ModelConstraintDefine;
import com.jiuqi.dna.core.def.model.ModelConstructorDefine;
import com.jiuqi.dna.core.def.model.ModelDeclarator;
import com.jiuqi.dna.core.def.model.ModelDeclare;
import com.jiuqi.dna.core.def.model.ModelInvokeDefine;
import com.jiuqi.dna.core.def.model.ModelInvokeStage;
import com.jiuqi.dna.core.def.model.ModelObjSourceDefine;
import com.jiuqi.dna.core.def.model.ModelPropAccessDefine;
import com.jiuqi.dna.core.def.model.ModelPropertyDefine;
...
public abstract class ModelServiceBase<TMO> extends ServiceBase<Context> {
	...
	public ModelServiceBase(String title) {
		super(title);
		this.moClass = (Class<TMO>) TypeArgFinder.get(this.getClass(), ModelServiceBase.class, 0);
	}
	...
	protected abstract class ModelServiceBinder<TModelDeclaror extends ModelDeclarator<TMO>> {
		...
		public ModelServiceBinder() {
			this.declarorClass = (Class) TypeArgFinder.get(this.getClass(), ModelServiceBinder.class, 0);
		}
		...
		protected final void internalBind(
			...
			ModelPropertyAccessor accessor = ModelServiceBase.this.propertyAccessors.get(accessorClass);
			if (accessor == null) {
				throw new IllegalArgumentException("找不到" + accessorClass + "对应的属性访问器");
			}
			((ModelPropAccessDefineImpl) propAccessDefine).internalSetAccessor(accessor);
		}
		...
		protected final void internalBind(ModelConstructorDefine constructor,
			...
			ModelConstructor<?> handler = ModelServiceBase.this.constructors.get(constructorClass);
			if (handler == null) {
				throw new IllegalArgumentException("找不到" + constructorClass + "对应的构造器");
			}
			((ModelConstructorDefineImpl) constructor).internalSetConstructor(handler);
		}
		...
		protected final void internalBind(ModelActionDefine action,
			...
			ModelActionHandler<?> handler = ModelServiceBase.this.actionHandlers.get(handlerClass);
			if (handler == null) {
				throw new IllegalArgumentException("找不到" + handlerClass + "对应的动作处理器");
			}
		...
		protected final void internalBind(ModelConstraintDefine constraint,
			...
			ModelConstraintChecker checker = ModelServiceBase.this.constraintCheckers.get(constraintClass);
			if (checker == null) {
				throw new IllegalArgumentException("找不到" + constraintClass + "对应的约束检查器");
			}
			((ModelConstraintDefineImpl) constraint).internalSetChecker(checker);
		}
		...
		protected final void internalBind(ModelObjSourceDefine source,
			...
			ModelObjProvider<?> provider = ModelServiceBase.this.objProviders.get(providerClass);
			if (provider == null) {
				throw new IllegalArgumentException("找不到" + providerClass + "对应的模型实体提供器");
			}
	...
	public abstract class ModelActionHandler<TAO> extends ModelInvokee {
		...
		protected ModelActionHandler() {
			this.aoClass = (Class<TAO>) TypeArgFinder.get(this.getClass(), ModelActionHandler.class, 0);
		}
	...
	public abstract class ModelPropertyAccessor extends ModelInvokee {
		...
		public ModelPropertyAccessor() {
			...
			for (Class<?> clazz = this.getClass(); clazz != ModelPropertyAccessor.class; clazz = clazz.getSuperclass()) {
				for (Method method : clazz.getDeclaredMethods()) {
					String name = method.getName();
					if (name.startsWith("doSet")) {
						DataType type = TypeFactory.rootTypeOf(name.substring(5));
			...
			if (setSupports != null) {
				this.setSupports = setSupports.toArray(new DataType[setSupports.size()]);
			} else {
				this.setSupports = DataTypeBase.emptyArray;
			}
			if (getSupports != null) {
				this.getSupports = getSupports.toArray(new DataType[getSupports.size()]);
			} else {
	...
	public abstract class ModelObjProvider<TAO> extends ModelInvokee {
		...
		public ModelObjProvider() {
			this.aoClass = (Class<TAO>) TypeArgFinder.get(this.getClass(), ModelObjProvider.class, 0);
		}
	...
	final Map<Class<?>, ModelActionHandler<?>> actionHandlers = new HashMap<Class<?>, ModelActionHandler<?>>(0);
	final Map<Class<?>, ModelPropertyAccessor> propertyAccessors = new HashMap<Class<?>, ModelPropertyAccessor>(0);
	final Map<Class<?>, ModelConstructor<?>> constructors = new HashMap<Class<?>, ModelConstructor<?>>(0);
	final Map<Class<?>, ModelConstraintChecker> constraintCheckers = new HashMap<Class<?>, ModelConstraintChecker>(0);
	final Map<Class<?>, ModelObjProvider<?>> objProviders = new HashMap<Class<?>, ModelObjProvider<?>>(0);
	...
	static abstract class ModelInvokeeFiller {
		...
		public final void regModelInvokee(ModelServiceBase<?> modelService,
				Class<?> invokeeClass) {
			((Map) this.getMap(modelService)).put(invokeeClass, modelService.newObjectInNode(invokeeClass, null, null));
		}
	...
	private static final ModelInvokeeFiller actionHandlerFiller = new ModelInvokeeFiller(ModelServiceBase.ModelActionHandler.class) {
		@Override
		@SuppressWarnings("unchecked")
		public void fill(ModelServiceBase<?> modelService, List list,
				Class<?> aoClass) {
			if (!modelService.actionHandlers.isEmpty()) {
				for (ModelServiceBase<?>.ModelActionHandler<?> h : modelService.actionHandlers.values()) {
					if (aoClass == null || h.aoClass == None.class || h.aoClass.isAssignableFrom(aoClass)) {
						list.add(h);
					}
				}
			}
		...
		public boolean matchFillModelInvokee(Object key1, Object key2,
			...
			if (key1 == null) {
				...
				return key3 == null;
			} else if (key3 == null && key1 instanceof Class<?> && key2 instanceof Class<?>) {
				return true;
			} else {
				return false;
			}
	...
	private static final ModelInvokeeFiller propertyAccessorFiller = new ModelInvokeeFiller(ModelServiceBase.ModelPropertyAccessor.class) {
		@Override
		@SuppressWarnings("unchecked")
		public void fill(ModelServiceBase<?> modelService, List list,
				Class<?> aoClass) {
	...
	private static final ModelInvokeeFiller constructorFiller = new ModelInvokeeFiller(ModelServiceBase.ModelConstructor.class) {
		@Override
		@SuppressWarnings("unchecked")
		public void fill(ModelServiceBase<?> modelService, List list,
				Class<?> aoClass) {
			if (!modelService.constructors.isEmpty()) {
				for (ModelServiceBase<?>.ModelConstructor<?> c : modelService.constructors.values()) {
					if (aoClass == null || c.aoClass == None.class || c.aoClass.isAssignableFrom(aoClass)) {
						list.add(c);
					}
				}
			}
	...
	private static final ModelInvokeeFiller constraintCheckerFiller = new ModelInvokeeFiller(ModelServiceBase.ModelConstraintChecker.class) {
		@Override
		@SuppressWarnings("unchecked")
		public void fill(ModelServiceBase<?> modelService, List list,
				Class<?> aoClass) {
			if (!modelService.constraintCheckers.isEmpty()) {
				for (ModelServiceBase<?>.ModelConstraintChecker c : modelService.constraintCheckers.values()) {
					list.add(c);
				}
			}
		}
	...
	private static final ModelInvokeeFiller objProviderFiller = new ModelInvokeeFiller(ModelServiceBase.ModelObjProvider.class) {
		@Override
		@SuppressWarnings("unchecked")
		public void fill(ModelServiceBase<?> modelService, List list,
				Class<?> aoClass) {
			if (!modelService.objProviders.isEmpty()) {
				for (ModelServiceBase<?>.ModelObjProvider<?> h : modelService.objProviders.values()) {
					if (aoClass == null || h.aoClass == None.class || h.aoClass.isAssignableFrom(aoClass)) {
						list.add(h);
					}
				}
	...
	private static ModelInvokeeFiller[] modelInvokeeFilters = { actionHandlerFiller, propertyAccessorFiller, constructorFiller, constraintCheckerFiller, objProviderFiller };
	...
	final boolean tryRegDeclaredClasses(Class<?> serviceClass,
		...
		if (ModelServiceBinder.class.isAssignableFrom(declaredClass)) {
			ModelServiceBinder<?> binder = (ModelServiceBinder<?>) this.newObjectInNode(declaredClass, null, null);
	...
	final void afterRegInvokees(Mode servicePublishMode,
		...
		for (ModelServiceBinder<?> binder = this.bindlers; binder != null; binder = binder.next) {
			ModelDeclarator<TMO> declaror = this.findElement(binder.declarorClass);
			// TODO REG to ModelDeclarorClass
			if (declaror != null) {
				try {
					((ModelServiceBinder) binder).doBind(declaror, (ModelDeclare) declaror.getDefine());
				} catch (Throwable e) {
