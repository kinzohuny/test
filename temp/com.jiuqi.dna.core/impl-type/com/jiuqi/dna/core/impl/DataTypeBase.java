...
import com.jiuqi.dna.core.impl.StaticStructDefineImpl.ETryLoadJavaFieldsAndPrepareAccessInfo;
...
public abstract class DataTypeBase implements DataTypeInternal {
	...
	public Object convert(Object from) {
		...
		if (!this.javaClass.isInstance(from)) {
			throw new ValueConvertException("无法将对象\"" + from + "\"转换成" + this + "类型");
		}
		return from;
	}
	...
	public final ArrayDataTypeBase arrayOf() {
		...
		if (this.javaClass == null) {
			throw new UnsupportedOperationException(this.toString() + "类型不支持其数组类型");
		}
		...
		try {
			...
			if (old != null) {
				if (old != dataType) {
					...
					System.err.println("类型注册失败：ID冲突\"" + old + "\" - \"" + dataType + "\"，ID[" + id + "]");
				} else {
				...
				if (javaClass != null) {
					...
					if (old != null) {
						if (old != dataType) {
							...
							System.err.println("类型注册失败：Java类冲突\"" + old + "\" - \"" + dataType + "\"，class[" + javaClass + "]");
						} else {
							System.err.println("类型注册警告：类型\"" + dataType + "\"被多次注册");
						}
						return false;
	...
	final static DataTypeInternal dataTypeOfJavaClass(Class<?> javaClass,
		...
		try {
			...
			} else if (rejectStruct || (jcm & (Modifier.ABSTRACT | Modifier.INTERFACE)) != 0) {
				return new RefDataType(javaClass);
			} else {
				try {
					return new StaticStructDefineImpl(javaClass, dataTypesByClass, staticStructDefineByName, false);
				} catch (Throwable e) {
	...
	public final static DataTypeInternal dataTypeOfJavaObj(Object obj) {
		...
		return dataTypeOfJavaClass(obj.getClass(), false);
	}
	 ...
	 */
	@Deprecated
	final static DataTypeInternal dataTypeOfJavaObjForOldSerial(Object obj) {
		if (obj == null) {
			throw new NullArgumentException("obj");
		}
		if (obj instanceof DynObj) {
		...
		DataTypeInternal dti = dataTypeOfJavaClass(obj.getClass(), false);
		if (dti instanceof RefDataType) {
			return ((RefDataType) dti).tryGetOldSerialDataType();
		}
		return dti;
	}
	 ...
	 */
	@Deprecated
	final static DataTypeInternal dataTypeOfJavaClassForOldSerial(Class<?> clazz) {
		DataTypeInternal dti = dataTypeOfJavaClass(clazz, false);
		if (dti instanceof RefDataType) {
			return ((RefDataType) dti).tryGetOldSerialDataType();
		}
		return dti;
	...
	private final static UnsupportedOperationException unsupportedStaticStructDefine(
		...
		if (cause != null) {
			return new UnsupportedOperationException("Java类型\"" + javaClass + "\"不支持DNA结构类型", cause);
		} else {
			return new UnsupportedOperationException("Java类型\"" + javaClass + "\"不支持DNA结构类型");
		}
	}

	private final static StaticStructDefineImpl findOrGetStaticStructDefine(
			Class<?> javaClass, boolean findOrGet, boolean force) {
		StaticStructDefineImpl def = findOrGetStaticStructDefineInternel(javaClass, findOrGet, force);
		if (force && !def.isAllJavaFields) {
			AssertionError ex = new AssertionError("当前操作期望类型[" + def.name + "]所有字段都是StructField，与实际情况冲突，建议对类型标注@StructClass");
			Logger logger = DNALogManager.getLogger("core/service");
			logger.logError(null, ex, false);
			if (ApplicationImpl.IN_DEBUG_MODE) {
				ex.printStackTrace();
			}
		}
		return def;
	...
	private final static StaticStructDefineImpl findOrGetStaticStructDefineInternel(
			Class<?> javaClass, boolean findOrGet, boolean force) {
		if (javaClass == null) {
			throw new NullArgumentException("javaClass");
		}
		...
		try {
			...
			Throwable ex = null;
			try {
				return new StaticStructDefineImpl(javaClass, dataTypesByClass, staticStructDefineByName, force);
	 ...
	 */
	public final static StructDefineImpl tryGetStructDefineOf(Object obj) {
		if (obj == null) {
			throw new NullArgumentException("obj");
		}
		if (obj instanceof DynObj) {
		...
		return findOrGetStaticStructDefine(obj.getClass(), true, false);
	}
	...
	static final long ENTRY_TYPE_VOID = 0x000E000000000000L;
	...
	final static GUID calcComponentTypeID(GUID typeID) {
		final long msb = typeID.getMostSigBits();
		if ((msb & ARRAY_DEM_MASK) == 0) {
			return typeID;
		}
		return GUID.valueOf(msb & ARRAY_TYPE_MASK, typeID.getLeastSigBits());
	}
	...
	// 该方法与DataTypeBase.calcArryTypeID方法与对数组维度值的处理是相同的。
	final static short calcArrayTypeDimension(GUID arrayTypeID) {
		return (short) ((arrayTypeID.getMostSigBits() >>> 56) + 1);
	}

	final static GUID calcNumTypeID(byte precision, byte scale) {
		return GUID.valueOf((PRIMITIVE_TYPE_DOUBLE & 0xFFFFL) << 48, (precision & 0xFFL) << 8 | scale & 0xFFL);
	}
	...
	final static GUID calcStrTypeID(boolean isFixed, boolean isLOB,
		...
		return GUID.valueOf(ENTRY_TYPE_STRING, (l << 32) | (length & 0xFFFFFFFFL));
	}
	...
	final static GUID calcBytesTypeID(boolean isFixed, boolean isLOB, int length) {
		...
		return GUID.valueOf(ENTRY_TYPE_BYTES, (l << 32) | (length & 0xFFFFFFFFL));
	}
