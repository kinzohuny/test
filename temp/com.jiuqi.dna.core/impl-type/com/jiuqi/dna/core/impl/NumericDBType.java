...
public final class NumericDBType extends DoubleType {
	...
	public final boolean canDBTypeConvertTo(DataType target) {
		if (target == this || target == FloatType.TYPE || target == DoubleType.TYPE) {
			return true;
		}
	...
	public final NumberTypePrecedence getPrecedence() {
		return NumberTypePrecedence.NUMERIC;
	}
	...
	public final static DataTypeMap map = new DataTypeMap() {
		...
		final int keyCode(int length, int precision, int scale) {
			...
			if (MAX_PRECISION < precision) {
				throw new IllegalArgumentException(MAX_PRECISION + " < precision");
			}
	...
	static {
		DataTypeUndigester.regUndigester(new DataTypeUndigester(TypeCodeSet.NUMERIC_H) {
			@Override
	...
	NumericDBType(int precision, int scale) {
		...
		this.max = Math.pow(10, this.precision - this.scale);
		this.min = -this.max;
		regDataType(this);
	}
