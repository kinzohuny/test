...
import com.jiuqi.dna.core.type.ResourceType;
...
public final class ResourceTypeImpl<TFacade> extends DataTypeBase implements
	...
	static {
		DataTypeUndigester.regUndigester(new DataTypeUndigester(TypeCodeSet.RESOURCE_H) {
	...
	public static <TFacade> ResourceTypeImpl<TFacade> RESOURCE(
		...
		try {
			if (modifyVersion != mv) {
				for (ResourceTypeImpl type = last != null ? last : resourceTypes.get(facadeClass); type != null; type = type.next) {
					if (type.category == category) {
						return type;
					}
				}
			}
			ResourceTypeImpl type = new ResourceTypeImpl<TFacade>(facadeClass, category);
			if (last != null) {
				last.next = type;
			} else {
