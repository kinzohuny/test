...
final class DataTypeIDMap {
		...
		try {
			...
			if (++this.mapSize >= this.rehashSize) {
				...
				for (MapEntry entry : this.map) {
					while (entry != null) {
						final int newHashIndex = entry.dataType.getID().hashCode() / newMapCapacity_1;
	...
	final DataType get(final GUID dataTypeID) {
		...
		this.readLock.lock();
		try {
			return this.internalGet(dataTypeID.hashCode() / this.mapCapacity_1, dataTypeID);
		} finally {
	...
	private final void internalPut(final DataType dataType) {
		final int hashIndex = dataType.getID().hashCode() / this.mapCapacity_1;
		this.map[hashIndex] = new MapEntry(dataType, this.map[hashIndex]);
	}
