...
import com.jiuqi.dna.core.def.info.InfoGroupDeclare;
	...
	public final void ensurePrepared(ContextImpl<?, ?, ?> context,
		...
		if (!this.isPrepared || rePrepared) {
			if (this.declarator != null) {
				for (LanguagePackage lp = context.session.application.findInfoGroupLanguages(this.declarator.getClass().getName()); lp != null; lp = lp.next) {
					for (int i = 0, c = lp.nameMessages.length; i < c; i += 2) {
						final InfoDefineImpl info = this.infos.find(lp.nameMessages[i]);
						if (info != null) {
							info.putLanguageMessage(lp.localeKey, lp.nameMessages[i + 1]);
						}
					}
				}
			}
