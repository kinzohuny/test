...
import com.jiuqi.dna.core.def.info.ErrorInfoDeclare;
import com.jiuqi.dna.core.def.info.HintInfoDeclare;
import com.jiuqi.dna.core.def.info.InfoKind;
import com.jiuqi.dna.core.def.info.ProcessInfoDeclare;
import com.jiuqi.dna.core.def.info.WarningInfoDeclare;
 ...
 * 
 */
class InfoDefineImpl extends NamedDefineImpl implements ErrorInfoDeclare,
		HintInfoDeclare, WarningInfoDeclare, ProcessInfoDeclare {
	 ...
	 * {4,0,3,2,-1,4,0,3,2}
	 */
	private volatile short[] frmtInfo;
	...
	private final short[] ensureFormatInfo() {
		...
		if (frmtInfo == null) {
			synchronized (this) {
				if ((frmtInfo = this.frmtInfo) == null) {
	...
	private LocaleInfoImpl findLocaleInfo(int localeKey) {
		for (LocaleInfoImpl lang = this.langs; lang != null; lang = lang.next) {
			if (lang.localeKey == localeKey) {
				return lang;
			}
		}
		return null;
	...
	final void putLanguageMessage(int localeKey, String message) {
		if (this.findLocaleInfo(localeKey) == null) {
			this.langs = new LocaleInfoImpl(localeKey, message, this.langs);
		}
	}
	...
	short[] buildFrmtInfo(String message, boolean appendParam) {
		...
		for (int i = 0, start = 0, pStart = -1; i < frmtL; i++) {
			...
			case '}':
				...
				int index = this.parameters.indexOfName(message, pStart, pEnd - pStart);
				if (index < 0) {
					tryAppendParam: {
						if (appendParam && pEnd != i) {
							// 指定类型的参数，尝试追加参数
							final String typeName = message.substring(pEnd + 1, i);
							final DataType pType = TypeFactory.typeOf(typeName, null);
							if (pType != UnknownType.TYPE) {
								InfoParameterDefineImpl param = new InfoParameterDefineImpl(this, message.substring(pStart, pEnd), pType);
	...
	final void formatMessage(int localeKey, Appendable to, Object p1,
			Object p2, Object p3, Object[] params) {
		if (to == null) {
			throw new NullArgumentException("to");
		}
		this.format(localeKey, to, p1, p2, p3, params, true);
	}

	final String formatMessage(int localeKey, Object p1, Object p2, Object p3,
			Object[] params) {
		return this.format(localeKey, null, p1, p2, p3, params, true);
	}

	final void formatMessage(int localeKey, Appendable to, Object[] params) {
		if (to == null) {
			throw new NullArgumentException("to");
		}
		this.format(localeKey, to, null, null, null, params, false);
	}

	final String formatMessage(int localeKey, Object[] params) {
		return this.format(localeKey, null, null, null, null, params, false);
	}
	 ...
	 *            参数是否规格化过
	 */
	private String format(int localeKey, Appendable to, Object p1, Object p2,
		...
		findLocale: {
			if (localeKey != 0) {
				final LocaleInfoImpl l = this.findLocaleInfo(localeKey);
				if (l != null) {
					this.ensureFormatInfo();
					aMessage = l.message;
	...
	public final InfoParameterDefineImpl newParameter(String name, DataType type) {
		InfoParameterDefineImpl param = new InfoParameterDefineImpl(this, name, type);
		this.parameters.add(param);
		return param;
	}
	...
	void merge(SXElement element, SXMergeHelper helper) {
		...
		this.isReportToUser = element.getBoolean(xml_attr_user, this.isReportToUser);
		for (SXElement paramElement = element.firstChild(xml_element_params, InfoParameterDefineImpl.xml_element_name_param); paramElement != null; paramElement = paramElement.nextSibling(InfoParameterDefineImpl.xml_element_name_param)) {
			String fn = paramElement.getAttribute(NamedDefineImpl.xml_attr_name, null);
			InfoParameterDefineImpl param = this.parameters.find(fn);
			if (param == null) {
				param = new InfoParameterDefineImpl(this, fn, paramElement.getAsType(FieldDefineImpl.xml_attr_type, helper.querier));
				this.parameters.add(param);
			}
