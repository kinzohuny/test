...
import com.jiuqi.dna.core.spi.log.LogTask;
...
final class LogManager {
	...
	final void setState(int newState) {
		ok: synchronized (this) {
			...
			case S_DISABLE:
				...
				case S_PREPARING:
					break;
				case S_READY:
					this.site.application.overlappedManager.postWork(this.logWork = new LogWork());
					break;
				default:
					break ok;
				}
		...
		throw new IllegalArgumentException("illegal state: old[" + this.state + "],new[" + newState + "]");
	}
	...
	private final void loggerDoLog(WorkingThread thread) {
		thread.setName("dna-logger-" + this.site.name);
		final LogTask logTask = new LogTask(this);
		try {
			while (this.loggerWaitToLog()) {
				final ContextImpl<?, ?, ?> context = LogManager.this.site.newSystemSessionSiteContext(ContextKind.TRANSIENT);
				try {
