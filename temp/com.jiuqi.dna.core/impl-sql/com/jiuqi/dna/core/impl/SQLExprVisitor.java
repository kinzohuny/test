...
import com.jiuqi.dna.core.impl.NValueExpr.ResultType;
import com.jiuqi.dna.core.spi.sql.SQLAliasUndefinedException;
import com.jiuqi.dna.core.spi.sql.SQLColumnNotFoundException;
import com.jiuqi.dna.core.spi.sql.SQLHierarchyNotFoundException;
...
import com.jiuqi.dna.core.spi.sql.SQLVariableUndefinedException;
...
class SQLExprVisitor extends VisitorBase<SQLExprContext> {
	...
	public void visitBinaryExpr(SQLExprContext visitorContext, NBinaryExpr e) {
		...
		case ADD:
			...
			case ENUM:
				...
				break;
			default:
				throw new SQLNotSupportedException(e.left.startLine(),
						e.left.startCol(), "'+'运算符不能应用于该类型的操作数");
			}
			break;
		case SUB:
	...
	public void visitAggregateExpr(SQLExprContext visitorContext,
		...
		case COUNT:
			if (value != null) {
				visitorContext.valueExpr = e.quantifier == SetQuantifier.DISTINCT ? SQLFunc
						.xCountDistinct(value) : SQLFunc.xCount(value);
			} else {
		...
		case AVG:
			visitorContext.valueExpr = e.quantifier == SetQuantifier.DISTINCT ? SQLFunc
					.xAvgDistinct(value) : SQLFunc.xAvg(value);
			break;
		case SUM:
			visitorContext.valueExpr = e.quantifier == SetQuantifier.DISTINCT ? SQLFunc
					.xSumDistinct(value) : SQLFunc.xSum(value);
			break;
		case MAX:
	...
	@Override
	public void visitAnalyticFunction(SQLExprContext visitorContext,
			NAnalyticFunctionExpr e) {
		ValueExpr[] partitions;
		if (e.partitions != null && e.partitions.length > 0) {
			partitions = new ValueExpr[e.partitions.length];
			for (int i = 0; i < e.partitions.length; i++) {
				partitions[i] = (ValueExpr) visitorContext
						.build(e.partitions[i]);
			}
		} else {
			partitions = null;
		}
		OrderByItemImpl[] orderbys = new OrderByItemImpl[e.orders.length];
		for (int i = 0; i < e.orders.length; i++) {
			NOrderByColumn c = e.orders[i];
			ValueExpression ov = visitorContext.build(c.column);
			orderbys[i] = new OrderByItemImpl(null, (ValueExpr) ov);
			if (!c.asc) {
				orderbys[i].setDesc(true);
			}
		}
		switch (e.func) {
		case SUM:
			ValueExpression value = visitorContext.build(this, e.column);
			if (value instanceof ValueExpr) {
			} else {
				throw new SQLSyntaxException("分析函数不支持的表达式。");
			}
			AnalyticFunctionExpr.WindowType type = e.windowType != null ? e.windowType
					.change() : AnalyticFunctionExpr.WindowType.ROWS;
			Bound preceding = bound(visitorContext, e.preceding,
					AnalyticFunctionExpr.UNBOUNDED);
			Bound following = bound(visitorContext, e.following, null);
			visitorContext.valueExpr = new AnalyticFunctionExpr(
					AnalyticFunction.SUM, (ValueExpr) value, partitions,
					orderbys, type, preceding, following);
			break;
		case ROW_NUMBER:
			visitorContext.valueExpr = new AnalyticFunctionExpr(
					AnalyticFunction.ROW_NUMBER, partitions,
					orderbys);
			break;
		case RANK:
			visitorContext.valueExpr = new AnalyticFunctionExpr(
					AnalyticFunction.RANK, partitions,
					orderbys);
			break;
		case DENSE_RANK:
			visitorContext.valueExpr = new AnalyticFunctionExpr(
					AnalyticFunction.DENSE_RANK, partitions,
					orderbys);
			break;
		}
	}

	private static final AnalyticFunctionExpr.Bound bound(
			SQLExprContext visitorContext, Object obj,
			AnalyticFunctionExpr.Bound def) {
		if (obj == null) {
			return def;
		} else if (obj == NAnalyticFunctionExpr.CURRENT_ROW) {
			return AnalyticFunctionExpr.CURRENT_ROW;
		} else if (obj == NAnalyticFunctionExpr.UNBOUNDED) {
			return AnalyticFunctionExpr.UNBOUNDED;
		} else if (obj instanceof NValueExpr) {
			return new AnalyticFunctionExpr.LimitBound(
					(ValueExpr) visitorContext.build((NValueExpr) obj));
		} else {
		...
		try {
			if (e.params != null && e.params.length > 0) {
				...
				if (pattern == null) {
					...
					throw new SQLNotSupportedException(e.startLine(),
							e.startCol(), "函数'" + e.func.toString()
									+ "'不接受参数类型为(" + sb.toString() + ")的调用");
				}
	...
	public void visitQuerySpecific(SQLExprContext visitorContext,
		...
		if (s.select.columns.length != 1) {
			throw new SQLNotSupportedException(s.select.startLine(),
					s.select.startCol(), "参与运算的子查询只能包含一列");
		}
	...
	public void visitCompareExpr(SQLExprContext visitorContext, NCompareExpr e) {
		...
		try {
			...
			throw new SQLNotSupportedException(e.left.endLine(),
					e.left.endCol(), ex.getMessage());
		}
	}
	...
	public void visitBetweenExpr(SQLExprContext visitorContext, NBetweenExpr e) {
		...
		try {
			...
			throw new SQLNotSupportedException(e.value.endLine(),
					e.value.endCol(), ex.getMessage());
		}
	}
	...
	public void visitInExpr(SQLExprContext visitorContext, NInExpr e) {
		...
		if (e.param instanceof NInParamValueList) {
			...
			throw new SQLNotSupportedException(e.param.startLine(),
					e.param.startCol(), "不支持的操作数类型");
		}
	}
