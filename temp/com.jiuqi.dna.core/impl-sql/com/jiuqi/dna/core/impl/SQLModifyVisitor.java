...
import com.jiuqi.dna.core.spi.sql.SQLAliasDuplicateException;
...
import com.jiuqi.dna.core.spi.sql.SQLRelationNotFoundException;
import com.jiuqi.dna.core.spi.sql.SQLSyntaxException;
import com.jiuqi.dna.core.spi.sql.SQLTableNotFoundException;
...
class SQLModifyVisitor extends SQLDmlVisitor<SQLModifyContext> {
	...
	private TString getMasterTableName(NSource s) {
		...
		if (src instanceof NSourceRelate) {
			...
			return this.getMasterTableName(r.source);
		}
	...
	private static final String getModifyTargetAlias(NSource source) {
		if (source instanceof NSourceTable) {
			NSourceTable s = (NSourceTable) source;
			if (s.alias != null) {
				return s.alias.value;
			}
			return s.source.name.value;
		} else if (source instanceof NSourceJoin) {
			NSourceJoin j = (NSourceJoin) source;
			return getModifyTargetAlias(j.left);
		} else if (source instanceof NSourceRelate) {
			NSourceRelate r = (NSourceRelate) source;
			return getModifyTargetAlias(r.source);
		}
	...
	public void visitUpdateDeclare(SQLModifyContext visitorContext,
		...
		UpdateStatementImpl update = new UpdateStatementImpl(u.name.value,
				getModifyTargetAlias(u.body.update.source), table,
				(UpdateStatementDeclarator) visitorContext.declarator);
	...
	public void visitDeleteDeclare(SQLModifyContext visitorContext,
		...
		DeleteStatementImpl delete = new DeleteStatementImpl(d.name.value,
				getModifyTargetAlias(d.body.delete.source), table,
				(DeleteStatementDeclarator) visitorContext.declarator);
	...
	public void visitInsertSubQuery(SQLModifyContext visitorContext,
		...
		boolean found = false;
		if (q.query == null) {
			throw new SQLSyntaxException(q.startLine(), q.startCol(),
					"缺少select语句");
		}
		TableDefineImpl table = insert.moTableRef.target;
	...
	public void visitSourceTable(SQLModifyContext visitorContext, NSourceTable t) {
		...
		if (visitorContext.returnRef == null) {
			// 主表已经在sql语句初始化时建好，直接退出
			visitorContext.rootAlias = alias;
			visitorContext.returnRef = stmt.moTableRef;
			return;
		}
		if (stmt.findRelationRef(alias) != null) {
			if (t.alias != null) {
		...
		visitorContext.returnRef = visitorContext.returnRef.newJoin(table,
				alias);
	}
