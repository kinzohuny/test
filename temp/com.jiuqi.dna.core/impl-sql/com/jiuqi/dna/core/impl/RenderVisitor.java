...
import com.jiuqi.dna.core.impl.NLogicalExpr.Operator;
import com.jiuqi.dna.core.impl.NStrCompareExpr.Keywords;
...
class RenderVisitor implements SQLVisitor<StringFormatter> {
	...
	public void visitAnalyticFunction(StringFormatter f, NAnalyticFunctionExpr e) {
		f.append(e.func.toString()).append('(');
		if (e.column != null) {
			f.append(e.column);
		}
		f.append(')').append(" OVER (");
		if (e.partitions != null) {
			f.append("PARTITION BY ").singleline(e.partitions, ',');
		}
		f.append(" ORDER BY ");
		f.append(e.orders[0].column).space().append(e.orders[0].asc);
		for (int i = 1, c = e.orders.length; i < c; i++) {
			f.append(',').append(e.orders[i].column).space()
					.append(e.orders[i].asc);
		}
		if (e.windowType != null) {
			f.space().append(e.windowType.toString());
			if (e.following != null) {
				f.append(" BETWEEN ");
				if (e.preceding == NAnalyticFunctionExpr.CURRENT_ROW) {
					f.append("CURRENT ROW");
				} else if (e.preceding == NAnalyticFunctionExpr.UNBOUNDED) {
					f.append("UNBOUNDED PRECEDING");
				} else {
					f.append((NValueExpr) e.preceding).append(" PRECEDING");
				}
				f.append(" AND ");
				if (e.following == NAnalyticFunctionExpr.CURRENT_ROW) {
					f.append("CURRENT ROW");
				} else if (e.following == NAnalyticFunctionExpr.UNBOUNDED) {
					f.append("UNBOUNDED FOLLOWING");
				} else {
					f.append((NValueExpr) e.following).append(" FOLLOWING");
				}
			} else {
				if (e.preceding == NAnalyticFunctionExpr.CURRENT_ROW) {
					f.append("CURRENT ROW");
	...
	public void visitColumnRefExpr(StringFormatter f, NColumnRefExpr e) {
		f.append(f.encodeID(e.source.value)).append('.')
				.append(f.encodeID(e.field.value));
	}
	...
	public void visitHaidExpr(StringFormatter f, NHaidExpr e) {
		f.append("h_aid(").append(e.source.value).append('.')
				.append(e.path.value);
		if (e.offset != null) {
			f.space().append(e.relOrAbs ? "rel" : "abo").space()
					.append(e.offset);
		}
	}

	public void visitHlvExpr(StringFormatter f, NHlvExpr e) {
		f.append("h_lv(").append(e.source.value).append('.')
				.append(e.path.value);
	}
	...
	public void visitSearchedCaseExpr(StringFormatter f, NSearchedCaseExpr e) {
		...
		if (e.branches != null && e.branches.length > 0) {
			...
			for (int i = 1, c = e.branches.length; i < c; i++) {
				f.newline().append("WHEN ").append(e.branches[i].condition)
						.append(" THEN ").append(e.branches[i].returnValue);
			}
		}
	...
	public void visitSimpleCaseExpr(StringFormatter f, NSimpleCaseExpr e) {
		...
		if (e.branches != null && e.branches.length > 0) {
			f.append("WHEN ").append(e.branches[0].value).append(" THEN ")
					.append(e.branches[0].returnValue);
			for (int i = 1, c = e.branches.length; i < c; i++) {
				f.newline().append("WHEN ").append(e.branches[i].value)
						.append(" THEN ").append(e.branches[i].returnValue);
			}
		}
		if (e.elseBranch != null) {
	...
	public void visitIsLeafExpr(StringFormatter f, NIsLeafExpr e) {
		f.append(f.encodeID(e.left.value)).append(" IS LEAF ")
				.append(f.encodeID(e.hier.value));
	}
	...
	public void visitStrCompareExpr(StringFormatter f, NStrCompareExpr e) {
		f.append(e.first).append(e.not ? " NOT" : "").space()
				.append(e.keyword.toString()).space().append(e.second);
	...
	public void visitQueryStmt(StringFormatter f, NQueryStmt q) {
		...
		if (q.predefines != null && q.predefines.length > 0) {
			f.append("WITH").newline().indent().append('(')
					.append(q.predefines[0].query).append(") AS ")
					.append(q.predefines[0].name.value);
			for (int i = 1, c = q.predefines.length; i < c; i++) {
				f.append(',').newline().append('(')
						.append(q.predefines[i].query).append(") AS ")
						.append(q.predefines[i].name.value);
			}
			f.newline().back();
		}
		f.append(q.expr);
		if (q.order != null) {
			...
			if (q.order.columns != null && q.order.columns.length > 0) {
				f.append(q.order.columns[0].column).space()
						.append(q.order.columns[0].asc);
				for (int i = 1, c = q.order.columns.length; i < c; i++) {
					f.append(',').append(q.order.columns[i].column).space()
							.append(q.order.columns[i].asc);
				}
			}
			f.back();
		}
	...
	public void visitQuerySpecific(StringFormatter f, NQuerySpecific s) {
		...
		if (s.select.columns != null && s.select.columns.length > 0) {
			f.append(s.select.columns[0].expr);
			if (s.select.columns[0].alias != null) {
				f.append(" AS ").append(
						f.encodeID(s.select.columns[0].alias.value));
			}
		...
		f.back().newline().append("FROM").newline().indent()
				.multiline(s.from.sources, ',').back();
		if (s.where != null) {
			f.newline().append("WHERE ").newline().indent()
					.append(s.where.expr).back();
		}
		if (s.group != null) {
			f.newline().append("GROUP BY").newline().indent()
					.multiline(s.group.columns, ',');
		...
		if (s.having != null) {
			f.newline().append("HAVING").newline().indent()
					.append(s.having.expr).back();
		}
		f.back();
		if (s.unions != null) {
	...
	public void visitSourceRelate(StringFormatter f, NSourceRelate r) {
		...
		f.append(" RELATE ").append(f.encodeID(r.table.value)).append('.')
				.append(f.encodeID(r.rel.value));
		if (r.alias != null) {
	...
	public void visitSourceSubQuery(StringFormatter f, NSourceSubQuery q) {
		f.append('(').append(q.query).append(')').append(" AS ")
				.append(f.encodeID(q.alias.value));
	}
	...
	public void visitUpdateDeclare(StringFormatter f, NUpdateDeclare u) {
		f.append("DEFINE UPDATE ").append(f.encodeID(u.name.value)).append('(')
	...
	public void visitUpdateStmt(StringFormatter f, NUpdateStmt u) {
		...
		if (u.set.columns != null && u.set.columns.length > 0) {
			f.append(u.set.columns[0].column.value).append(" = ")
					.append(u.set.columns[0].value);
			for (int i = 1, c = u.set.columns.length; i < c; i++) {
				f.append(',').append(u.set.columns[i].column.value)
						.append(" = ").append(u.set.columns[i].value).newline();
			}
		}
		f.back();
		if (u.where != null) {
			if (u.where.expr != null) {
				f.newline().append("WHERE").newline().indent()
						.append(u.where.expr).back();
			} else {
				f.newline().append("WHERE CURRENT OF ")
						.append(u.where.cursor.value);
			}
		}
		f.append(';');
	}
	...
	public void visitDeleteStmt(StringFormatter f, NDeleteStmt d) {
		...
		if (d.where != null) {
			...
			if (d.where.expr != null) {
				...
				f.indent().append(" WHERE CURRENT OF ")
						.append(d.where.cursor.value).back();
			}
		}
		f.append(';');
	}
	...
	private void appendTableField(StringFormatter f, NTableField field) {
		...
		if (field.foreignKey != null) {
			f.append(" RELATION ").append(field.foreignKey.name.value)
					.append(" TO ").append(field.foreignKey.refTable.value)
					.append('.').append(field.foreignKey.refField.value);
		}
	}
	...
	private void appendTableBlock(StringFormatter f, NAbstractTableDeclare t) {
		f.newline().append("BEGIN").newline().indent().append("FIELDS")
				.newline().indent();
		if (t.primary.fields != null && t.primary.fields.length > 0) {
		...
		if (t.extend != null) {
			for (NTableExtend ext : t.extend) {
				f.append("FIELDS ON ").append(ext.name.value).newline()
						.indent();
				if (ext.fields != null && ext.fields.length > 0) {
		...
		if (t.index != null && t.index.length > 0) {
		...
		if (t.relation != null && t.relation.length > 0) {
			f.append("RELATIONS").newline().indent();
			f.append(t.relation[0].name.value).append(" TO ")
					.append(t.relation[0].target.value).append(" ON ")
					.append(t.relation[0].expr);
			for (int i = 1, c = t.relation.length; i < c; i++) {
				f.append(',').newline();
				f.append(t.relation[i].name.value).append(" TO ")
						.append(t.relation[i].target.value).append(" ON ")
						.append(t.relation[i].expr);
			}
			f.newline().back();
		}
		if (t.hierarchy != null && t.hierarchy.length > 0) {
			f.append("HIERARCHIES").newline().indent();
			f.append(t.hierarchy[0].name.value).append(" MAXLEVEL(")
					.append(t.hierarchy[0].limit.value).append(')');
			for (int i = 1, c = t.hierarchy.length; i < c; i++) {
				f.append(',').newline();
		...
		if (t.partition != null) {
			...
			f.append(')').newline().indent().append("VALVE ")
					.append(t.partition.size.value).append(" MAXCOUNT ")
					.append(t.partition.limit.value).newline().back();
		}
		f.back().append("END");
	}
	...
	public void visitProcedureDeclare(StringFormatter f, NProcedureDeclare p) {
		f.append("DEFINE PROCEDURE ").append(f.encodeID(p.name.value))
				.append('(').mark().multiline(p.params, ',').append(')')
				.newline().back().append("BEGIN").newline().indent()
				.multiline(p.stmts, StringFormatter.EMPTY_DELIMITER).newline()
				.back().append("END");
	}
	...
	public void visitFunctionDeclare(StringFormatter f, NFunctionDeclare func) {
		f.append("DEFINE FUNCTION ").append(f.encodeID(func.name.value))
				...
				.multiline(func.stmts, StringFormatter.EMPTY_DELIMITER)
				.newline().back().append("END");
	}

	public void visitSegment(StringFormatter f, NSegment s) {
		f.append("BEGIN").newline().indent()
				.multiline(s.stmts, StringFormatter.EMPTY_DELIMITER).newline()
				.back().append("END");
	}
	...
	public void visitAssignStmt(StringFormatter f, NAssignStmt a) {
		if (a.vars != null && a.vars.length > 0) {
			if (a.vars.length == 1) {
		...
		if (a.query != null) {
			f.append(a.query);
		} else if (a.values != null && a.values.length > 0) {
	...
	public void visitForeachStmt(StringFormatter f, NForeachStmt fe) {
		...
		if (fe.query != null) {
			...
			f.append(fe.call.name.value).append('(')
					.singleline(fe.call.params, ',').append(')');
		}
		f.append(" LOOP").newline();
