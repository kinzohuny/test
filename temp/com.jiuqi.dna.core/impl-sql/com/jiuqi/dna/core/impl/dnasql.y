...
set_func:
	sum_func { $$ = $1; }
	| 'AVG' '(' set_quantifier_op value_expr ')' {
			SetQuantifier q = (SetQuantifier)$3;
			$$ = new NAggregateExpr(NAggregateExpr.Func.AVG, (Token)$1, (Token)$5, (NValueExpr)$4, q == null ? SetQuantifier.ALL : q);
		}
	| 'MIN' '(' value_expr ')' {
			$$ = new NAggregateExpr(NAggregateExpr.Func.MIN, (Token)$1, (Token)$4, (NValueExpr)$3, SetQuantifier.ALL);
		}
	| 'MAX' '(' value_expr ')' {
			$$ = new NAggregateExpr(NAggregateExpr.Func.MAX, (Token)$1, (Token)$4, (NValueExpr)$3, SetQuantifier.ALL);
		}
	| 'COUNT' '(' set_quantifier_op value_expr ')' {
			SetQuantifier q = (SetQuantifier)$3;
			$$ = new NAggregateExpr(NAggregateExpr.Func.COUNT, (Token)$1, (Token)$5, (NValueExpr)$4, q == null ? SetQuantifier.ALL : q);
		}
	| 'COUNT' '(' '*' ')' {
			$$ = new NAggregateExpr(NAggregateExpr.Func.COUNT, (Token)$1, (Token)$4, null, SetQuantifier.ALL);
		}
	/*error*/
	| 'AVG' '(' set_quantifier_op value_expr error {
			after($4, new SQLTokenNotFoundException(")"));
			$$ = NValueExpr.EMPTY;
		}
	| 'MIN' '(' value_expr error {
			after($3, new SQLTokenNotFoundException(")"));
			$$ = NValueExpr.EMPTY;
		}
	| 'MAX' '(' value_expr error {
			after($3, new SQLTokenNotFoundException(")"));
			$$ = NValueExpr.EMPTY;
		}
	| 'COUNT' '(' '*' error {
...
sum_func:
	'SUM' '(' set_quantifier_op value_expr ')' {
			SetQuantifier q = (SetQuantifier)$3;
			$$ = new NAggregateExpr(NAggregateExpr.Func.SUM, (Token)$1, (Token)$5, (NValueExpr)$4, q == null ? SetQuantifier.ALL : q);
		}
	/*error*/
	| 'SUM' '(' set_quantifier_op value_expr error {
			after($4, new SQLTokenNotFoundException(")"));
			$$ = NValueExpr.EMPTY;
		}
	| 'SUM' error {
...
orderby:
	...
	/* error */
	| 'ORDER' 'BY' 
		error { after($2, new SQLSyntaxException("缺少列表达式")); }
	| 'ORDER' 
		error { after($1, new SQLTokenNotFoundException("BY")); }
	;
...
query_column:
	...
	| analytic_function_expr 'AS' error {
			after($2, new SQLSyntaxException("缺少别名"));
			$$ = NQueryColumn.EMPTY;
		}
	| analytic_function_expr error {
			after($1, new SQLSyntaxException("缺少别名"));
			$$ = NQueryColumn.EMPTY;
		}
	;

analytic_function_expr:
	sum_func 'OVER' '(' af_sum_partition_op orderby af_sum_window_clause_op ')' {
			NAggregateExpr sum = (NAggregateExpr)$1;
			if (!(sum.expr instanceof NColumnRefExpr)) {
				...
				NOrderBy orders = (NOrderBy)$5;
				NAnalyticFunctionExpr.NWindowClause w = (NAnalyticFunctionExpr.NWindowClause)$6;
				if (w == null) {
					$$ = new NAnalyticFunctionExpr(NAnalyticFunctionExpr.Func.SUM, (TextLocalizable)$1, (Token)$7, (NColumnRefExpr)sum.expr, partitions, orders.columns, null, null, null);
				} else {
					$$ = new NAnalyticFunctionExpr(NAnalyticFunctionExpr.Func.SUM, (TextLocalizable)$1, (Token)$7, (NColumnRefExpr)sum.expr, partitions, orders.columns, w.windowType, w.preceding, w.following);
				}
			}
		}
	| 'ROW_NUMBER' '(' ')' 'OVER' '(' af_sum_partition_op orderby ')' {
			LinkList l = (LinkList)$6;
			NValueExpr[] partitions;
			if (l != null) {
				partitions = l.toArray(new NValueExpr[l.count()]);
			} else {
			...
			NOrderBy orders = (NOrderBy)$7;
			$$ = new NAnalyticFunctionExpr(NAnalyticFunctionExpr.Func.ROW_NUMBER, (TextLocalizable)$1, (Token)$8, partitions, orders.columns);
		}
	| 'RANK' '(' ')' 'OVER' '(' af_sum_partition_op orderby ')' {
			LinkList l = (LinkList)$6;
			NValueExpr[] partitions;
			if (l != null) {
				partitions = l.toArray(new NValueExpr[l.count()]);
			...
			NOrderBy orders = (NOrderBy)$7;
			$$ = new NAnalyticFunctionExpr(NAnalyticFunctionExpr.Func.RANK, (TextLocalizable)$1, (Token)$8, partitions, orders.columns);
		}
	| 'DENSE_RANK' '(' ')' 'OVER' '(' af_sum_partition_op orderby ')' {
			LinkList l = (LinkList)$6;
			NValueExpr[] partitions;
			if (l != null) {
				partitions = l.toArray(new NValueExpr[l.count()]);
			...
			NOrderBy orders = (NOrderBy)$7;
			$$ = new NAnalyticFunctionExpr(NAnalyticFunctionExpr.Func.DENSE_RANK, (TextLocalizable)$1, (Token)$8, partitions, orders.columns);
		}
	/*error*/
	| sum_func 'OVER' '(' af_sum_partition_op orderby error {
			at($3, new SQLTokenNotFoundException(")"));
	  		$$ = null;
		}
	| sum_func 'OVER' '(' error {
			after($3, new SQLSyntaxException("缺少OrderBy子句"));
	  		$$ = null;
		}
	| sum_func 'OVER' error {
			after($2, new SQLTokenNotFoundException("("));
	...
	| 'ROW_NUMBER' '(' ')' 'OVER' '(' af_sum_partition_op orderby error {
			at($6, new SQLTokenNotFoundException(")"));
	  		$$ = null;
		}
	| 'ROW_NUMBER' '(' ')' 'OVER' '(' error {
			after($5, new SQLSyntaxException("缺少OrderBy子句"));
	  		$$ = null;
		}
	| 'ROW_NUMBER' '(' ')' 'OVER' {
			after($4, new SQLTokenNotFoundException("("));
	...
	| 'ROW_NUMBER' '(' ')' {
			after($3, new SQLTokenNotFoundException("OVER"));
	  		$$ = null;
		}
	| 'ROW_NUMBER' '(' {
			after($2, new SQLTokenNotFoundException(")"));
	...
	| 'RANK' '(' ')' 'OVER' '(' af_sum_partition_op orderby error {
			at($6, new SQLTokenNotFoundException(")"));
	  		$$ = null;
		}
	| 'RANK' '(' ')' 'OVER' '(' error {
	...
	| 'DENSE_RANK' '(' ')' 'OVER' '(' af_sum_partition_op orderby error {
			at($6, new SQLTokenNotFoundException(")"));
	  		$$ = null;
		}
	| 'DENSE_RANK' '(' ')' 'OVER' '(' error {
			after($5, new SQLSyntaxException("缺少OrderBy子句"));
	  		$$ = null;
		}
	| 'DENSE_RANK' '(' ')' 'OVER' {
...
af_sum_partition_op:
	'PARTITION' 'BY' af_sum_partition_list { $$ = $3; }
	| { $$ = null; }
	/*error*/
	| 'PARTITION' 'BY' error {
			after($2, new SQLSyntaxException("缺少表达式"));
			$$ = null;
		}
	| 'PARTITION' error {
			after($1, new SQLTokenNotFoundException("BY"));
			$$ = null;
		}
	;

af_sum_partition_list:
...
af_sum_window_clause_op:
	af_sum_window_clause { $$ = $1; }
	| { $$ = null; }
	;

af_sum_window_clause:
	af_sum_window_type af_sum_preceding {
			$$ = new NAnalyticFunctionExpr.NWindowClause((NAnalyticFunctionExpr.WindowType)$1, $2, null);
		}
	| af_sum_window_type 'BETWEEN' af_sum_preceding 'AND' af_sum_following {
			$$ = new NAnalyticFunctionExpr.NWindowClause((NAnalyticFunctionExpr.WindowType)$1, $3, $5);
		}
	;

af_sum_window_type:
	'ROWS' { $$ = NAnalyticFunctionExpr.WindowType.ROWS; }
	| 'RANGE' { $$ = NAnalyticFunctionExpr.WindowType.RANGE; }
	;

af_sum_preceding:
	'UNBOUNDED' 'PRECEDING' { $$ = NAnalyticFunctionExpr.UNBOUNDED; }
	| 'CURRENT' 'ROW' { $$ = NAnalyticFunctionExpr.CURRENT_ROW; }
	| value_expr 'PRECEDING' { $$ = $1; }
	/*error*/
	| 'UNBOUNDED' error {
  			after($1, new SQLTokenNotFoundException("PRECEDING"));
	  		$$ = null;
  		}
	| 'CURRENT' error {
  			after($1, new SQLTokenNotFoundException("ROW"));
	  		$$ = null;
  		}
	| value_expr error  {
  			after($1, new SQLTokenNotFoundException("PRECEDING"));
...
af_sum_following:
	'UNBOUNDED' 'FOLLOWING' { $$ = NAnalyticFunctionExpr.UNBOUNDED; }
	| 'CURRENT' 'ROW' { $$ = NAnalyticFunctionExpr.CURRENT_ROW; }
	| value_expr 'FOLLOWING' { $$ = $1; }
	/*error*/
	| 'UNBOUNDED' error {
  			after($1, new SQLTokenNotFoundException("FOLLOWING"));
	  		$$ = null;
  		}
	| 'CURRENT' error {
  			after($1, new SQLTokenNotFoundException("ROW"));
