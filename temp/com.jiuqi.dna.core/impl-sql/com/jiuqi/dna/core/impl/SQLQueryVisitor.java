...
import com.jiuqi.dna.core.spi.sql.SQLClassNotFoundException;
...
class SQLQueryVisitor extends SQLDmlVisitor<SQLQueryContext> {
			...
			if (name != null) {
				if (q.getColumns().find(name) != null) {
					if (c.alias != null) {
						...
						throw new SQLAliasDuplicateException(c.startLine(),
								c.startCol(), name);
					}
				}
				q.newColumn(e, name);
	...
	public void visitQuerySpecific(SQLQueryContext visitorContext,
			NQuerySpecific s) {
		this.visitQuerySpecific(visitorContext, s, new SQLExprContext(
	...
	public void visitSourceSubQuery(SQLQueryContext visitorContext,
		...
		visitorContext.build(this, sub, q.query,
				ContextVariableIntl.derived_query_standalone ? null
						: visitorContext.resolver);
