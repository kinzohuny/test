 ...
 * 
 */
public final class TableUsage {

	final TableDefineImpl target;

	final BitSet dbTableUsage = new BitSet();

	final BitSet hierarchyUsage = new BitSet();

	public TableUsage(TableRef tableRef) {
	...
	public final void useTables(BitSet bits) {
			...
			public Iterator<DBTableDefineImpl> iterator() {
				return new Iterator<DBTableDefineImpl>() {
					...
					private DBTableDefineImpl findNext(int from) {
						int index = TableUsage.this.dbTableUsage.nextSetBit(from);
						if (index >= 0) {
					...
					public DBTableDefineImpl next() {
						...
						int index = TableUsage.this.target.dbTables.indexOf(current);
			...
			public Iterator<HierarchyDefineImpl> iterator() {
				return new Iterator<HierarchyDefineImpl>() {
					...
					private HierarchyDefineImpl findNext(int from) {
						int index = TableUsage.this.hierarchyUsage.nextSetBit(from);
						if (index >= 0) {
							return TableUsage.this.target.hierarchies.get(index);
						}
						return null;
					}
