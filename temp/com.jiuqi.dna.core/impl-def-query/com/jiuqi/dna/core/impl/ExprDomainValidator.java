...
import com.jiuqi.dna.core.internal.da.statement.expr.IllegalValueExprDomainException;
...
final class ExprDomainValidator extends TraversedExprVisitor<RelationRefDomain> {
	...
	public final void visitSelectColumnRef(SelectColumnRefImpl expr,
		...
		if (expr.queryRef != domain.findRelationRefRecursively(expr.queryRef.getName())) {
			throw new IllegalValueExprDomainException(expr);
		}
	}
	...
	public void visitTableFieldRef(TableFieldRefImpl expr,
			RelationRefDomain domain) {
		if (expr.tableRef != domain.findRelationRefRecursively(expr.tableRef.getName())) {
			throw new IllegalValueExprDomainException(expr);
		}
	}
