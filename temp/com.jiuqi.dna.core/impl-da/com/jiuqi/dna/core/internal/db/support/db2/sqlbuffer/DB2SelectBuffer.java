...
class DB2SelectBuffer implements ISqlSelectBuffer {
	...
	public final DB2ExprBuffer groupby() {
		if (this.groups == null) {
	...
	public final DB2OrderExprBuffer orderby(boolean desc) {
		if (this.orders == null) {
	...
	public final DB2UnionedSelectBuffer union(boolean all) {
		if (this.unions == null) {
	...
	final void select(SqlStringBuffer sql, List<ParameterPlaceholder> args) {
		sql.append("select ");
		if (this.distinct) {
			sql.append("distinct ");
		}
	...
	final void from2union(SqlStringBuffer sql, List<ParameterPlaceholder> args) {
		if (this.dummy) {
	...
	private final void groups(SqlStringBuffer sql,
			List<ParameterPlaceholder> args) {
