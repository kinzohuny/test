...
final class DB2QueryBuffer extends SqlCommandBuffer implements ISqlQueryBuffer {
	...
	private final void limit(SqlStringBuffer sql,
			List<ParameterPlaceholder> args) {
		if (this.offset != null) {
	...
	private final void query(SqlStringBuffer sql,
			List<ParameterPlaceholder> args) {
	...
	private final void orders(SqlStringBuffer sql,
			List<ParameterPlaceholder> args) {
		for (int i = 0, c = this.orders.size(); i < c; i++) {
			if (i > 0) {
				sql.append(',');
			}
			this.orders.get(i).writeTo(sql, args);
		}
	}
