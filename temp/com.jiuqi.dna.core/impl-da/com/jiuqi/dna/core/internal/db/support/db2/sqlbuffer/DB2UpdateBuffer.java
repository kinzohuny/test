...
final class DB2UpdateBuffer extends SqlCommandBuffer implements
	...
	public final void writeTo(SqlStringBuffer sql,
		...
		if (this.target.joins == null) {
			...
			final String T = "\"$T\"";
			sql.append("update ").append(this.target.table).append(' ').append(T).append(" set ");
			if (this.assignFromSlaveTable) {
				...
				final String RN = "\"$RN\"";
				sql.append(",").append(this.target.alias).append(".\"RECID\" ").append(ID);
				...
				sql.append(") ").append(N).append(" where ").append(N).append('.').append(ID).append("=").append(T).append(".\"RECID\" and ").append(N).append('.').append(RN).append("<=1)");
			} else {
				for (int i = 0, c = this.values.size(); i < c; i++) {
					if (i > 0) {
						sql.append(',');
			...
			sql.append(this.target.alias).append(".\"RECID\"=").append(T).append(".\"RECID\")");
		}
	}
}
