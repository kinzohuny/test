...
final class ORMAccessorProxy<TEntity> implements ORMAccessor<TEntity>,
		TransientProxy<ORMAccessorImpl<TEntity>> {
	...
	public final int fetchIntoLimit(List<TEntity> into, long offset,
			...
			Object... argValues) {
		return (int) this.orm.fetchLimitInto(into, offset, rowCount, entityFactory, argValues);
	}
	...
	public final ORMAccessorImpl<TEntity> getProvider() {
		return this.orm;
	}

	public ORMAccessorProxy(ContextImpl<?, ?, ?> context,
			MappingQueryStatementImpl statement) {
		this.orm = new ORMAccessorImpl<TEntity>(context, statement, this);
	}
}