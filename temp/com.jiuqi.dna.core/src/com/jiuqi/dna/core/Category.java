 ...
 * 
 * 类别作为一种标识，是有一定的类型要求的，主要原则是便于使用，同时要支持DNA-Core框架中所定义的序列化。
...
public interface Category {
	 ...
	 *      2. 任何枚举类型；
	 *      3. 原始类型的装箱类型；
	 *      4. com.jiuqi.dna.core.type.GUID。
	 * </pre>
	 * 
	...
	public static final class Helper {
		...
		/**
		 * 支持的类型参见 <code>com.jiuqi.dna.core.Category.getIdentifier()</code>
		 * 方法的注释。
		 * 
		 * @param idType
		 *            要检查的标识符的类型。
		...
		public static boolean isSupportedIdType(Class<?> idType) {
			...
			return (idType == String.class || Enum.class.isAssignableFrom(idType) || idType == GUID.class || idType == Integer.class || idType == Long.class || idType == Short.class || idType == Byte.class || idType == Character.class || idType == Double.class || idType == Float.class || idType == Boolean.class);
		}
	}
}
