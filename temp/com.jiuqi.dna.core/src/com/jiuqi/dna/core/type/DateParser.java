...
public final class DateParser {
	...
	/**
	 * 解析日期时间字符串,并将其转换为DNA时间(效率较低,不建议频繁使用)<br>
	 * 待解析的日期时间字符串中,各元素(年 月 日 时 分 秒 毫秒)之间 <br>
	 * 需用一个指定的字符隔开,如: <br>
	 * 2000-02-29 <br>
	 * 2000-02-29 10:12:54 <br>
	 ...
	 * 
	 * @param dateTime
	 *            待格式化的长整形日期时间,可以是JAVA时间,也可以是DNA时间
	 ...
	 *            待格式化的长整形日期时间,可以是JAVA时间,也可以是DNA时间
	 * @param formatType
	 *            <li>DateParser.FORMAT_DATE: 只格式化日期部分</li> <li>
	 *            DateParser.FORMAT_DATE_TIME: 只格式化日期和时间部分</li> <li>
	 *            DateParser.FORMAT_DATE_TIME_MS: 格式化日期,时间和毫秒部分</li> <li>
	 *            DateParser.FORMAT_DATE_TIME_AUTOMS: 格式化日期,时间和毫秒部分,毫秒数为0,则不格式化</li>
	 *            <li>DateParser.FORMAT_TIME: 只格式化时间部分</li> <li>
	 *            DateParser.FORMAT_TIME_MS: 只格式化时间和毫秒部分</li> <li>
	 *            DateParser.FORMAT_TIME_AUTOMS: 格式化日期,时间和毫秒部分,毫秒数为0,则不格式化</li>
	 * @return 返回格式化后得到的日期时间字符串
	 */
	...
	public static long toDNADateTime(long dateTime) {
		...
		return baseDNATime | ((long) daysToDNADate((dateTime >= 0 || milliSeconds == 0) ? days + 1 : days) << DNATIME_DATEFILED_END_POS) | (msToDNATime(milliSeconds));
	}
	...
	// 中国夏令时结束时间
	// static final long[] DAYLIGHT_SAVING_TIME_END = new long[] {
	// -923130000000L,
	...
	private static void checkDivi(String s, int len, int position) {
		if (position >= len) {
			throw new ValueConvertException("日期时间不完整\t");
		}
		final char tempChar = s.charAt(position);
		if ('0' <= tempChar && tempChar <= '9') {
			throw new ValueConvertException("分界符错误,此处分界符应为非数字字符\t" + position);
		}
	}
	...
	private static int computeJavaDate(final int year, final int month,
		...
		if (date > DAYS_FOR_MONTH[month]) {
			throw new ValueConvertException(year + "年的" + month + "月,没有" + date + "天");
		}
		// 以公元2000年为基准
	...
	private static long computeJavaDateTime(String dateTimeStr) {
		...
		checkDivi(dateTimeStr, len, start++);
		if (start == len || !checkNumber(dateTimeStr, len, (dateTimeStr.charAt(start) == ' ' ? ++start : start))) {
			return days;
		}
	...
	private static long dnaDateTimeToJavaDateTime(long dnaDateTime) {
		...
		result = (tempYear * 365 + result) + DAYS_COUNT_FOR_MONTH[(int) ((dnaDateTime >>> DNATIME_MONTH_END_POS) & 0xF)] + (((dnaDateTime >>> DNATIME_DATE_END_POS) & DNATIME_DATE_MASK) - 1);
		if (year > 2000) {
			result = result + 1 - isLeapYear;
		}
		result *= 86400000L;
		return result + (((dnaDateTime >>> DNATIME_HOUR_END_POS) & DNATIME_HOUR_MASK) * 3600 + ((dnaDateTime >>> DNATIME_MINUTE_END_POS) & DNATIME_MINUTE_MASK) * 60 + ((dnaDateTime >>> DNATIME_SECOND_END_POS) & DNATIME_SECOND_MASK)) * 1000 + (dnaDateTime & DNATIME_MS_MASK);
	}
	...
	private static long timeAdjust(long time) {
		...
		if (new Date(time).getHours() != getHourByTime(time)) {
			time -= 3600000L;
		}
	...
	private static int getHourByTime(long time) {
		return (int) ((time + 8 * 3600 * 1000) / (3600 * 1000)) % 24; // XXX：GMT+8
	}
	...
	private static long reverseTimeAdjust(long javaTime) {
		// int len = DAYLIGHT_SAVING_TIME_START.length - 1;
		// if (!(javaTime < DAYLIGHT_SAVING_TIME_START[0] ||
		// DAYLIGHT_SAVING_TIME_END[len] < javaTime)) {
		// while (0 <= len) {
		...
		// }
		// }
		if (new Date(javaTime).getHours() != getHourByTime(javaTime)) {
			javaTime += 3600000L;
		}
	...
	private static int daysToDNADate(int days) {
		...
		if (days == 0) {
			return dnaDate | ((year - 1) << DNATIME_YEAR_REL_POS) | (12 << DNATIME_MONTH_REL_POS) | (31 << DNATIME_DATE_REL_POS);
		}
		int month = (days / 30) + 1;
		if (days <= DAYS_COUNT_FOR_MONTH[month]) {
			month--;
		}
		return dnaDate | (year << DNATIME_YEAR_REL_POS) | (month << DNATIME_MONTH_REL_POS) | ((days - DAYS_COUNT_FOR_MONTH[month]) << DNATIME_DATE_REL_POS);
	}
	...
	private static int msToDNATime(int ms) {
		...
		// 时区 00000 (无时区)
		return 0 | ((ms / 3600000) << DNATIME_HOUR_END_POS) | (((ms % 3600000) / 60000) << DNATIME_MINUTE_END_POS) | (((ms % 60000) / 1000) << DNATIME_SECOND_END_POS) | (ms % 1000);
	}
	...
	/**
	 * 判断长整型时间是否为合法的DNA时间<br>
	 * DNA时间定义长整型数的最高两位必须是01
	 */
	...
	private static void checkDNADateTime(long dnaDateTime) {
		if (!isDNADateTime(dnaDateTime)) {
			throw new ValueConvertException("非法的DNA时间");
		}
	}
	...
	public final static void main(String[] args) throws ParseException {
		...
		// String s = "1988-6-4 12:20:34";
		// long t = DateParser.parse(s);
		//
		// System.out.println(df.format(new Date(t)));
		// System.out.println(DateParser.format(t));
		...
		// Date d = df.parse(s);
		// System.out.println();
		// System.out.println(new Date(t).getHours());
		// System.out.println(df.format(new Date(t)));
		// System.out.println(DateParser.format(t));
	}
