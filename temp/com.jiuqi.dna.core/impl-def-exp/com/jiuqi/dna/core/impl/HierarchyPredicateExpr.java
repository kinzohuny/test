...
import com.jiuqi.dna.core.def.exp.HierarchyPredicate;
...
public class HierarchyPredicateExpr extends ConditionalExpr implements
	...
	public final ConditionalExpression not() {
		return new HierarchyPredicateExpr(!this.not, this.left, this.hierarchy, this.predicate, this.right, this.level);
	}
	...
	public final void render(SXElement element) {
		...
		element.setAttribute(HierarchyPredicateExpr.xml_attr_hrchy, this.hierarchy.name);
		element.setEnum(HierarchyPredicateExpr.xml_attr_hrchy_type, this.predicate);
		if (this.right != null) {
	...
	@Override
	public final HierarchyPredicateExpr clone(RelationRefDomain domain,
			ArgumentableDefine args) {
		TableRef left = (TableRef) domain.getRelationRefRecursively(this.left.getName());
		TableRef right = (TableRef) domain.getRelationRefRecursively(this.right.getName());
		ValueExpr level = this.level != null ? this.level.clone(domain, args) : null;
		return new HierarchyPredicateExpr(this.not, left, this.hierarchy, this.predicate, right, level);
	}
