...
public class TreeNodeImpl<TData> implements TreeNode<TData> {
	...
	private final void rangeCheck(int index) {
		if (index < 0 || this.childrenCount <= index) {
			throw new IndexOutOfBoundsException("Index:" + index + "childrenCount:" + this.childrenCount);
		}
	}

	final void sortChildren(final Comparator<? super TData> sortComparator) {
		if (sortComparator != null && this.childrenCount > 0) {
			SortUtil.sort(this.children, 0, this.childrenCount, new Comparator<TreeNodeImpl<TData>>() {
				public int compare(TreeNodeImpl<TData> a, TreeNodeImpl<TData> b) {
	...
	final void filterAndSortRecursively(TreeNodeFilter<? super TData> filter,
		...
		this.recursivelyFilterAndSort(filter, absoluteLevel, relativeLevel, sortComparator);
	}
	...
	private boolean recursivelyFilterAndSort(
		...
		if (filter != null) {
			if (this.parent != null || this.data != null) {
				Acception acc = filter.accept(this.data, absoluteLevel, relativeLevel);
				if (acc == null) {
					this.data = null;
					this.clear();
					return true;
		...
		if (this.childrenCount > 0) {
			...
			for (int i = this.childrenCount - 1; i >= 0; i--) {
				if (this.children[i].recursivelyFilterAndSort(filter, absoluteLevel, relativeLevel, sortComparator)) {
					this.remove(i);
				}
			}
		}
