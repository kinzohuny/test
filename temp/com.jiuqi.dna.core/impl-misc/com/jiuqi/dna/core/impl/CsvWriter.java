...
public class CsvWriter {
	...
	public void write(String content, boolean preserveSpaces)
		...
		if (!textQualify && this.userSettings.UseTextQualifier && (content.indexOf(this.userSettings.TextQualifier) > -1 || content.indexOf(this.userSettings.Delimiter) > -1 || (!this.useCustomRecordDelimiter && (content.indexOf(Letters.LF) > -1 || content.indexOf(Letters.CR) > -1)) || (this.useCustomRecordDelimiter && content.indexOf(this.userSettings.RecordDelimiter) > -1) || (this.firstColumn && content.length() > 0 && content.charAt(0) == this.userSettings.Comment) ||
		...
		if (textQualify) {
			...
			if (this.userSettings.EscapeMode == ESCAPE_MODE_BACKSLASH) {
				...
				content = replace(content, "" + this.userSettings.TextQualifier, "" + Letters.BACKSLASH + this.userSettings.TextQualifier);
			} else {
			...
			content = replace(content, "" + Letters.BACKSLASH, "" + Letters.BACKSLASH + Letters.BACKSLASH);
			content = replace(content, "" + this.userSettings.Delimiter, "" + Letters.BACKSLASH + this.userSettings.Delimiter);

			if (this.useCustomRecordDelimiter) {
				content = replace(content, "" + this.userSettings.RecordDelimiter, "" + Letters.BACKSLASH + this.userSettings.RecordDelimiter);
			} else {
	...
	private static String replace(String original, String pattern,
			String replace) {
