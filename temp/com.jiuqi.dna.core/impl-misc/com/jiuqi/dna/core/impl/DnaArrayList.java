...
public class DnaArrayList<E> extends AbstractList<E> implements List<E>,
	...
	final void adjust(Filter<? super E> filter,
			Comparator<? super E> sortComparator) {
		int ls = this.size;
		if (ls > 0) {
			if (filter != null) {
				int acceptedCount = 0;
				...
				if (acceptedCount == 0 && ls > 0) {
					...
					this.removeTail(acceptedCount);
					ls = acceptedCount;
				}
			}
			if (ls > 0 && sortComparator != null) {
				SortUtil.sort(this, sortComparator);
			}
		}
	}
}
