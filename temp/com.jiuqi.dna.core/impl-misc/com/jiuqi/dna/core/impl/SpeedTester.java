...
public final class SpeedTester {
	...
	private static class Nanos {
		...
		void testTimes(long minTimes, long maxNano) throws Throwable {
			double b = 1.0;
			for (this.times = minTimes; b >= 1.0; this.times = (long) (this.times * Math.min(Math.max(b, 1.1), 100.0))) {
				this.repeat();
		...
		final double testTimesPerNano(long maxNano) throws Throwable {
			...
			return (double) (this.workNano - this.comparisonNano) / (double) this.times;
		}

		@Override
		public final String toString() {
			return "<" + this.work.description() + "> : \r\n" + "\t测试耗时:[" + (double) this.workNano / this.times + "](Nano)\r\n" + "\t对比耗时:[" + (double) this.comparisonNano / this.times + "](Nano)\r\n" + "\t耗时差:[" + (double) (this.workNano - this.comparisonNano) / this.times + "](Nano)\r\n\t耗时比:[" + (double) (this.workNano - this.comparisonNano) / this.workNano * 100 + "]%";
		}
