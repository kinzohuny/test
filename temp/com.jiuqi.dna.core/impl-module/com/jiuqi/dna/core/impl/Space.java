...
import com.jiuqi.dna.core.impl.ModelServiceBase.ModelInvokee;
import com.jiuqi.dna.core.impl.ModelServiceBase.ModelInvokeeFiller;
...
import com.jiuqi.dna.core.misc.IteratorFilter;
import com.jiuqi.dna.core.service.Publish;
import com.jiuqi.dna.core.service.Service;
import com.jiuqi.dna.core.spi.def.DefineKind;
...
class Space extends SpaceNode implements SpaceToken {
					...
					if (ServiceBase.class.isAssignableFrom(e.targetClass)) {
						...
						while (sib != null) {
							if (sib instanceof SpaceElementBroker<?>) {
								((ServiceBase<?>) ((SpaceElementBroker<?>) sib).element).doDispose(context);
							}
							sib = sib.next;
						}
					}
	...
	final Space findSub(String spacePath, int offset, int len) {
		for (Space space = this.firstChild; space != null; space = space.next) {
			if (space.name.length() == len && spacePath.regionMatches(offset, space.name, 0, len)) {
				return space;
			}
		}
		return null;
	...
	final void regDeclarator(DeclaratorBase declarator,
		...
		SpaceElementBroker<DeclaratorBase> broker = new SpaceElementBroker<DeclaratorBase>(declarator, publishMode);
		this.regInvokee(declarator.getClass(), broker, catcher);
	}

	final void regService(ServiceBase<?> service, Publish.Mode publishMode,
			ExceptionCatcher catcher) {
		this.regInvokee(service.getClass(), new SpaceElementBroker<ServiceBase<?>>(service, publishMode), catcher);
		if (service instanceof ResourceServiceBase<?, ?, ?>) {
			final ResourceServiceBase<?, ?, ?> rs = (ResourceServiceBase<?, ?, ?>) service;
			this.regInvokee(rs.facadeClass, new ResourceServiceBroker(rs, publishMode), catcher);
			this.site.cache.putResourceService(rs);
		}
	}

	final void regNamedDefineToSpace(Class<?> defineIntfClass,
			NamedDefineImpl define, ExceptionCatcher catcher) {
		final NamedDefineBroker broker = (NamedDefineBroker) this.findInvokeeBase(defineIntfClass, String.class, null, null, ServiceInvokeeBase.MASK_DEFINE, InvokeeQueryMode.IN_SPACE);
		if (broker != null) {
			NamedDefineImpl exists = broker.putDefine(define);
			if (exists != null) {
				catcher.catchException(new IllegalArgumentException("重名的定义被注册:" + exists + " -> " + define), define);
			}
		} else {
			this.regInvokeeToSpace(defineIntfClass, new NamedDefineBroker<NamedDefineImpl>(defineIntfClass, this, define), catcher);
		}
	}
	...
	final void regNamedFactoryElement(NamedFactoryElementGather<?, ?> factory,
		...
		if (this.publishMode == null || publishMode == Publish.Mode.PROTECTED) {
			...
			final NamedFactoryElementBroker<?> broker = (NamedFactoryElementBroker<?>) this.findInvokeeBase(factoryClass, String.class, null, null, ServiceInvokeeBase.MASK_ELEMENT_META, InvokeeQueryMode.IN_SPACE);
			if (broker != null) {
				NamedFactoryElement old = broker.putElementMeta(meta);
				if (old != null) {
					catcher.catchException(new IllegalArgumentException("别名重复的元素被注册:" + meta + " -> " + old), meta);
				}
			} else {
				this.regInvokeeToSpace(factoryClass, new NamedFactoryElementBroker(factory, this, meta), catcher);
			}
		} else {
			if (publishMode == Publish.Mode.DEFAULT) {
				publishMode = this.publishMode;
			}
			this.space.regNamedFactoryElement(factory, meta, publishMode, catcher);
		}
	}
	...
	final boolean regDeclareScript(String declareName, DNASqlType type,
		...
		if (this.publishMode == null || publishMode == Publish.Mode.PROTECTED) {
			final DeclareScriptBroker broker = (DeclareScriptBroker) this.findInvokeeBase(URL.class, String.class, DNASqlType.class, null, ServiceInvokeeBase.MASK_DECLARE_SCRIPT, InvokeeQueryMode.IN_SPACE);
			if (broker != null) {
				URL old = broker.putDeclareScriptURL(declareName, type, url);
				if (old != null) {
					broker.putDeclareScriptURL(declareName, type, old);
					catcher.catchException(new IllegalArgumentException("重名的声明脚本被注册(" + declareName + "):\r\n-> " + url + "\r\n-> " + old), url);
					return false;
				}
			} else {
				this.regInvokeeToSpace(URL.class, new DeclareScriptBroker(this, declareName, type, url), catcher);
			}
			return true;
		} else {
			if (publishMode == Publish.Mode.DEFAULT) {
			...
			return this.space.regDeclareScript(declareName, type, url, publishMode, catcher);
		}
	}
	...
	final class InvokeeIterator implements
		...
		private final void doNext() {
			...
			do {
				...
				while (this.sie == null) {
					while (this.space.invokeeTable == null || this.index >= this.space.invokeeTable.length) {
	...
	@Override
	final EventListenerChain collectEvent(Class<?> eventClass, Object key1,
		...
		do {
			...
			for (Space space = this; space != null; space = space.space) {
				if (space.invokeeTableSize > 0) {
					for (ServiceInvokeeEntry e = space.invokeeTable[hash & (space.invokeeTable.length - 1)]; e != null; e = e.next) {
						if (e.targetClass == eventClass) {
							chain = mode.collectEvent(e, chain, key1, key2, key3);
							continue;
						}
					}
				}
	...
	@Override
	final ServiceInvokeeBase findInvokeeBase(Class<?> targetClass,
			Class<?> key1Class, Class<?> key2Class, Class<?> key3Class,
			int mask, InvokeeQueryMode mode) {
		final int hash = HashUtil.hash(targetClass);
		Space space = this;
		do {
			if (space.invokeeTableSize > 0) {
				for (ServiceInvokeeEntry e = space.invokeeTable[hash & (space.invokeeTable.length - 1)]; e != null; e = e.next) {
					if (e.targetClass == targetClass) {
						ServiceInvokeeBase invokee = mode.findInvokeeBase(e, key1Class, key2Class, key3Class, mask);
						if (invokee != null) {
							return invokee;
						}
						break;
	...
	@SuppressWarnings("unchecked")
	final <TDefine extends NamedDefine> void fillAllRuntimeDefines(
			Class<TDefine> defineIntfClass, List<TDefine> list,
			DataSourceRef dataSourceRef) {
		if (this.dataSourceRef == dataSourceRef) {
			final NamedDefineBroker broker = (NamedDefineBroker) this.findInvokeeBase(defineIntfClass, String.class, null, null, ServiceInvokeeBase.MASK_DEFINE, InvokeeQueryMode.IN_SPACE);
			if (broker != null) {
		...
		for (Space space = this.firstChild; space != null; space = space.next) {
			space.fillAllRuntimeDefines(defineIntfClass, list, dataSourceRef);
		}
	}
	...
	final ServiceInvokeeBase getTaskHandler(Class<?> taskClass,
		...
		ServiceInvokeeBase taskHandler = this.findInvokeeBase(taskClass, null, null, null, mask | ServiceInvokeeBase.MASK_TASK, mode);
		if (taskHandler == null) {
			throw new UnsupportedOperationException("找不到任务处理器[" + taskClass + '(' + taskMethod + ")]");
		}
		return taskHandler;
	}
	...
	final <TResult> ServiceInvokeeBase<TResult, Context, Object, Object, Object> findResultListProvider(
		...
		return this.findInvokeeBase(resultClass, key1Class, key2Class, key3Class, ServiceInvokeeBase.MASK_LIST, mode);
	}
	...
	final <TResult> ServiceInvokeeBase<TResult, Context, Object, Object, Object> findResultProvider(
		...
		return this.findInvokeeBase(resultClass, key1Class, key2Class, key3Class, ServiceInvokeeBase.MASK_RESULT, mode);
	}
	...
	final <TResult> ServiceInvokeeBase<TResult, Context, Object, Object, Object> findTreeNodeProvider(
		...
		return this.findInvokeeBase(elementClass, key1Class, key2Class, key3Class, ServiceInvokeeBase.MASK_TREE, mode);
	}
	...
	final ResourceServiceBase findResourceService(Class<?> facadeClass,
			InvokeeQueryMode mode) {
		ServiceInvokeeBase<ResourceServiceBase, Context, ?, ?, ?> invokeeBase = this.findInvokeeBase(facadeClass, null, null, null, ServiceInvokeeBase.MASK_RESOURCE, mode);
		if (invokeeBase != null) {
	...
	final void regInvokee(Class<?> targetClass, ServiceInvokeeBase invokee,
			ExceptionCatcher catcher) {
		if (this.publishMode == null || invokee.publishMode == Publish.Mode.PROTECTED) {
	...
	private final void regInvokeeToSpace(Class<?> targetClass,
		...
		if (entry == null) {
			entry = new ServiceInvokeeEntry(targetClass, hash, firstEntry, invokee);
	...
	private final <TResult, TKey1, TKey2> boolean tryFillModelInvokees(
		...
		ModelInvokeeFiller filler = ModelServiceBase.findModelInvokeeFiller(resultClass);
		if (filler == null) {
			return false;
		}
	...
	final <TResult, TKey1, TKey2, TKey3> boolean tryFillList(
		...
		if ((key1 == null || key2 == null && key1 instanceof DefineKind) && NamedDefine.class.isAssignableFrom(resultClass)) {
			// TODO 区分运行时和设计期 if (key == DefineKind.DESIGN)
			return this.fillRuntimeDefines((Class<NamedDefine>) resultClass, (List<NamedDefine>) list);
		}
		// 填充模型调用器
	...
	private final ModelInvokee findModelInvokeeByName(Class<?> invokeeClass,
			String className) {
		ModelInvokeeFiller filler = ModelServiceBase.getModelInvokeeFiller(invokeeClass);
		ModelServiceIterator mss = this.site.new ModelServiceIterator();
		while (mss.hasNext()) {
			ModelServiceBase<?> ms = mss.next();
			Map<Class<?>, ? extends ModelInvokee> mp = filler.getMap(ms);
			if (!mp.isEmpty()) {
				for (Map.Entry<Class<?>, ? extends ModelInvokee> mi : mp.entrySet()) {
					Class<?> c = mi.getKey();
					if (c.getName().equals(className) && invokeeClass.isAssignableFrom(c)) {
						return mi.getValue();
					}
				}
			}
		...
		if (key1 == null) {
			...
			if (key2 == null) {
				if (NamedDefine.class.isAssignableFrom(resultClass)) {
					return (TResult) this.findNamedDefine(resultClass, (String) key1);
				} else if (resultClass == Class.class) {
					return (TResult) this.site.application.tryLoadClass((String) key1);
				} else if (User.class == resultClass) {
					if (User.USER_NAME_ANONYM.equals(key1)) {
						return (TResult) BuildInUser.anonym;
					} else if (User.USER_NAME_DEBUGGER.equals(key1)) {
						return (TResult) BuildInUser.debugger;
					} else if (User.USER_NAME_SYSTEM.equals(key1)) {
						return (TResult) BuildInUser.system;
					}
				} else if (ModelInvokee.class.isAssignableFrom(resultClass)) {
					return (TResult) this.findModelInvokeeByName(resultClass, (String) key1);
				}
			} else if (key3 == null && key2 instanceof String) {
				if (resultClass == Class.class) {
					return (TResult) this.site.application.tryLoadClass((String) key1, (String) key2);
				}
			}
		}
		return null;
	...
	final class ModelServiceIterator
		...
		protected ModelServiceBase<?> filter(
			...
			if (in instanceof SpaceElementBroker<?>) {
				final Object element = ((SpaceElementBroker<?>) in).getElement();
	...
	final void fillService(List<Service> list) {
		ServiceIterator it = new ServiceIterator();
		while (it.hasNext()) {
			ServiceBase<?> s = it.next();
	...
	@SuppressWarnings("unchecked")
	final <T> void fillInvokees(List<T> list, Class<?> t) {
		if (this.invokeeTableSize == 0) {
			return;
		}
		for (ServiceInvokeeEntry e : this.invokeeTable) {
			for (ServiceInvokeeEntry e2 = e; e2 != null; e2 = e2.next) {
				for (ServiceInvokeeBase<?, ?, ?, ?, ?> invokee = e2.first; invokee != null; invokee = invokee.next) {
					if (t.isAssignableFrom(invokee.getClass())) {
						list.add((T) invokee);
					}
				}
			}
