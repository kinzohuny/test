...
import com.jiuqi.dna.core.def.table.TableEquiRelationDeclare;
...
final class TableEquiRelationDefineImpl extends TableRelationDefineImpl
	...
	TableEquiRelationDefineImpl(TableDefineImpl owner, String name,
		...
		this.condition = this.owner.selfRef.expOf(selfField).xEq(this.expOf(targetField));
	}
	...
	private synchronized final void rebuildCondition() {
		this.condition = this.owner.selfRef.expOf(this.selfField).xEq(this.expOf(this.targetField));
	}

	@Override
	final void merge(SXElement element, SXMergeHelper helper) {
		super.merge(element, helper);
		this.selfField = this.owner.fields.get(element.getString(xml_attr_self_field));
		this.targetField = this.target.fields.get(element.getString(xml_attr_target_field));
	}
