...
import com.jiuqi.dna.core.def.model.ModelFieldDeclare;
 ...
 * 
 */
public class StructFieldDefineImpl extends FieldDefineImpl implements
		ArgumentDeclare, ModelFieldDeclare, FieldValueAccessor {
	...
	final boolean isFieldValueNullNoCheck(Object so) {
		return this.accessor.memBytes == 0 && this.accessor.getObject(this.offset, so) == null;
	}
	...
	final void loadAccessorAndDefaultValue(int structFieldIndex) {
		...
		this.accessor = FieldAccessor.getAccessor(this.type, !this.isDynField);
	...
	static final Comparator<StructFieldDefineImpl> FIELD_NIO_SRLZ_CMP = new Comparator<StructFieldDefineImpl>() {
		public int compare(StructFieldDefineImpl o1, StructFieldDefineImpl o2) {
			final int o1i = (o1.type == BooleanType.TYPE) ? 1 : (o1.type instanceof ObjectDataType ? 2 : 0);
			final int o2i = (o2.type == BooleanType.TYPE) ? 1 : (o2.type instanceof ObjectDataType ? 2 : 0);
			return o1i - o2i;
		}
	};

	final void loadDefaultNoCheck(DynObj dynSO) {
		this.accessor.setValueD(this, this.offset, dynSO, this.defaultReadableValue);
	}

	final void loadDefaultNoCheck(Object so) {
		this.accessor.setValue(this, this.offset, so, this.defaultReadableValue);
	}

	final void assignNoCheck(Object src, Object dest, OBJAContext objaContext,
			DataTranslatorHelper<?, ?> dth) {
		if (this.isStateField) {
			this.accessor.assign(this, this.offset, src, dest, objaContext, dth);
		} else {
			this.accessor.setValue(this, this.offset, dest, this.defaultReadableValue);
		}
	}
	...
	final void assignNoCheck(DynObj src, DynObj dest, OBJAContext objaContext,
			DataTranslatorHelper<?, ?> dth) {
		if (this.isStateField) {
			this.accessor.assignD(this, this.offset, src, dest, objaContext, dth);
		} else {
			this.accessor.setValueD(this, this.offset, dest, this.defaultReadableValue);
		}
	}
	...
	final int initDynObjOffsets(int nullOffset) {
		if (this.accessor.memBytes == 0) {// Object field
			...
			if (this.isDynField) {
				this.offset = this.accessor.getBinDynFieldOffset(this.owner.binSize);
	...
	final void updateFieldInfo(Field javaField, StructField ano,
			AsTableField anTableField) {
		this.javaField = javaField;
		this.isDynField = false;
		this.offset = (int) Utils.objectFieldOffset(javaField);
		this.asTableField = anTableField;
		if (this.title == null || this.title.length() == 0) {
			this.title = ano != null ? Utils.noneNull(ano.title()) : anTableField != null ? Utils.noneNull(anTableField.title()) : "";
		}
		if (this.description == null || this.description.length() == 0) {
			this.description = ano != null ? Utils.noneNull(ano.description()) : anTableField != null ? Utils.noneNull(anTableField.description()) : "";
		}
		this.isReadonly = (Modifier.FINAL & javaField.getModifiers()) != 0;
		if (ano != null) {
			this.isStateField = ano.stateField();
			this.isRecursive = ano.recursive();
		} else {
			this.isStateField = (javaField.getModifiers() & Modifier.TRANSIENT) == 0;
			this.isRecursive = this.isStateField;
		}
	}
	...
	final boolean typeCompatible(DataType type) {
		final DataType targetRoot = type.getRootType();
		return this.type.getRootType() == targetRoot || (this.type instanceof EnumTypeImpl<?> && (targetRoot == StringType.TYPE || EnumTypeImpl.ordinalSupport(targetRoot)));
	}
	...
	public final void setFieldValue(Object so, ReadableValue value) {
		...
		if (dynSO != null) {
			this.accessor.setValueD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0 && value.isNull()) {
				this.setIsNullMask(dynSO, true);
			}
		} else {
			this.accessor.setValue(this, this.offset, so, value);
		}
	}
	...
	public final Object internalGet(Object so) {
		if (this.owner.isDynObj) {
			return this.accessor.getObjectD(this.offset, this.owner.toDynObj(so));
		} else {
	...
	public final void internalSet(Object so, Object value) {
		if (this.owner.isDynObj) {
			this.accessor.setObjectD(this, this.offset, this.owner.toDynObj(so), value);
		} else {
			this.accessor.setObject(this, this.offset, so, value);
		}
	}
	...
	final void setFieldValueNullNoCheck(DynObj dynSO) {
		this.accessor.setValueD(this, this.offset, dynSO, ReadableValue.NULL);
		if (this.nullOffset >= 0) {
			this.setIsNullMask(dynSO, true);
		}
	}
	...
	final void setFieldValueNullNoCheck(Object so) {
		this.accessor.setValue(this, this.offset, so, ReadableValue.NULL);
	}

	public final void SETLMergeLongValueNoCheck(DynamicObject entity, long value) {
		this.accessor.SETLMergeLongValueNoCheck(this, this.offset, entity, value);
	}

	public final void SETLMergeDoubleValueNoCheck(DynamicObject entity,
			double value) {
		this.accessor.SETLMergeDoubleValueNoCheck(this, this.offset, entity, value);
	}

	public final void SETLMergeNullNoCheck(DynamicObject entity) {
		if (this.accessor.SETLMergeNullNoCheck(this.offset, entity) && this.nullOffset >= 0) {
	...
	public final boolean getFieldValueAsBoolean(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getBooleanD(this.offset, dynSO) : this.accessor.getBoolean(this.offset, so);
	}
	...
	public final void setFieldValueAsBoolean(Object so, boolean value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setBooleanD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setBoolean(this, this.offset, so, value);
		}
	}
	...
	public final byte getFieldValueAsByte(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getByteD(this.offset, dynSO) : this.accessor.getByte(this.offset, so);
	}
	...
	public final void setFieldValueAsByte(Object so, byte value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setByteD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setByte(this, this.offset, so, value);
		}
	}
	...
	public final short getFieldValueAsShort(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getShortD(this.offset, dynSO) : this.accessor.getShort(this.offset, so);
	}
	...
	public final void setFieldValueAsShort(Object so, short value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setShortD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setShort(this, this.offset, so, value);
		}
	}
	...
	public final int getFieldValueAsInt(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getIntD(this.offset, dynSO) : this.accessor.getInt(this.offset, so);
	}
	...
	public final void setFieldValueAsInt(Object so, int value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setIntD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setInt(this, this.offset, so, value);
		}
	}
	...
	public final long getFieldValueAsLong(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getLongD(this.offset, dynSO) : this.accessor.getLong(this.offset, so);
	}
	...
	public final void setFieldValueAsLong(Object so, long value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setLongD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setLong(this, this.offset, so, value);
		}
	}
	...
	public final long getFieldValueAsDate(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getDateD(this.offset, dynSO) : this.accessor.getDate(this.offset, so);
	}
	...
	public final void setFieldValueAsDate(Object so, long value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setDateD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setDate(this, this.offset, so, value);
		}
	}
	...
	public final double getFieldValueAsDouble(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getDoubleD(this.offset, dynSO) : this.accessor.getDouble(this.offset, so);
	}
	...
	public final void setFieldValueAsDouble(Object so, double value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setDoubleD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setDouble(this, this.offset, so, value);
		}
	}
	...
	public final float getFieldValueAsFloat(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getFloatD(this.offset, dynSO) : this.accessor.getFloat(this.offset, so);
	}
	...
	public final void setFieldValueAsFloat(Object so, float value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setFloatD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setFloat(this, this.offset, so, value);
		}
	}
	...
	public final String getFieldValueAsString(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getStringD(this.offset, dynSO) : this.accessor.getString(this.offset, so);
	}
	...
	public final void setFieldValueAsString(Object so, String value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setStringD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setString(this, this.offset, so, value);
		}
	}
	...
	public final GUID getFieldValueAsGUID(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getGUIDD(this.offset, dynSO) : this.accessor.getGUID(this.offset, so);
	}
	...
	public final void setFieldValueAsGUID(Object so, GUID value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setGUIDD(this, this.offset, dynSO, value);
		} else {
			this.accessor.setGUID(this, this.offset, so, value);
		}
	}
	...
	public final byte[] getFieldValueAsBytes(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getBytesD(this.offset, dynSO) : this.accessor.getBytes(this.offset, so);
	}
	...
	public final void setFieldValueAsBytes(Object so, byte[] value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setBytesD(this, this.offset, dynSO, value);
		} else {
			this.accessor.setBytes(this, this.offset, so, value);
		}
	}
	...
	public final void setFieldValueAsObject(Object so, Object value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setObjectD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
	...
	public final char getFieldValueAsChar(Object so) {
		DynObj dynSO = this.owner.checkSO(so);
		return dynSO != null ? this.accessor.getCharD(this.offset, dynSO) : this.accessor.getChar(this.offset, so);
	}
	...
	public final void setFieldValueAsChar(Object so, char value) {
		DynObj dynSO = this.owner.checkSO(so);
		if (dynSO != null) {
			this.accessor.setCharD(this, this.offset, dynSO, value);
			if (this.nullOffset >= 0) {
				this.setIsNullMask(dynSO, false);
			}
		} else {
			this.accessor.setChar(this, this.offset, so, value);
		}
	}
	...
	public void render(SXElement element) {
		...
		element.setBoolean(StructFieldDefineImpl.xml_attr_transient, !this.isStateField);
	}

	@Override
	void merge(SXElement element, SXMergeHelper helper) {
		super.merge(element, helper);
		this.isStateField = !element.getBoolean(StructFieldDefineImpl.xml_attr_transient, !this.isStateField);
	}
	...
	final void writeOut(Object obj, InternalSerializer serializer)
		...
		if (this.owner.isDynObj) {
			...
			this.accessor.writeOutD(this, this.offset, dynObj, serializer);
		} else {
			this.accessor.writeOut(this, this.offset, obj, serializer);
		}
	}
	...
	final void readIn(Object obj, InternalDeserializer deserializer)
		...
		if (this.owner.isDynObj) {
			DynObj dynObj = this.owner.toDynObj(obj);
			this.accessor.readInD(this, this.offset, dynObj, deserializer);
		} else {
			this.accessor.readIn(this, this.offset, obj, deserializer);
		}
	}
	...
	static final StructFieldDefineImpl NIOSERIALIZE_OBJECTARRAY_FIELD = new StructFieldDefineImpl(true);

	static final StructFieldDefineImpl NIOSERIALIZE_CUSTOMDATA_FIELD = new StructFieldDefineImpl(false);
	...
	private StructFieldDefineImpl(final String name) {
		super(name, UnknownType.TYPE);
		this.owner = null;
		this.nextNIOSerializableField = this;
	}

	boolean serialize(final NSerializer serializer, final Object object) {
		if (this.owner.isDynObj) {
			return this.accessor.nioSerialize(serializer, this, (DynObj) object, this.offset);
		} else {
			return this.accessor.nioSerialize(serializer, this, object, this.offset);
		}
	}

	void unserialize(final NUnserializer unserializer, final Object object,
			final Object hint, DataTranslatorHelper<?, ?> dth) {
		if (this.owner.isDynObj) {
			this.accessor.nioUnserialize(unserializer, this, (DynObj) object, this.offset, hint, dth);
		} else {
			this.accessor.nioUnserialize(unserializer, this, object, this.offset, hint, dth);
		}
	}

	final Object internalGetFieldValueAsObject(final Object object) {
		if (this.owner.isDynObj) {
			return this.getFieldValueAsObject((DynamicObject) object);
		} else {
			return this.getFieldValueAsObject(object);
		}
	}

	static abstract class LoopStructFieldDefine extends StructFieldDefineImpl {
		protected LoopStructFieldDefine(final String name) {
			super(name);
		}
	}

	public ArgumentOutput output = ArgumentOutput.IN;
}