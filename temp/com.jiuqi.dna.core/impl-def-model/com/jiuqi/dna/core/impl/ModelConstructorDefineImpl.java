...
import com.jiuqi.dna.core.def.model.ModelConstructorDeclare;
import com.jiuqi.dna.core.impl.ModelServiceBase.ModelConstructor;
...
final class ModelConstructorDefineImpl extends ModelInvokeDefineImpl implements
	...
	final void internalSetConstructor(ModelConstructor constructor) {
		if (constructor != null && constructor != this.constructor) {
			...
			if (constructor.aoClass != None.class && !constructor.aoClass.isAssignableFrom(this.getAOClass())) {
	...
	final Object internalConstruct(ContextImpl<?, ?, ?> context, Object ao) {
		...
		try {
			if ((this.script == null || !this.script.executeScriptAsConstructor(context, mo, ao, this)) && this.constructor != null) {
				SpaceNode occorAtSave = this.constructor.getService().updateContextSpace(context);
				try {
	...
	public Object newMO(Context context) {
		return this.internalConstruct(ContextImpl.toContext(context), None.NONE);
	}
	...
	public final void render(SXElement element) {
		...
		if (this.constructor != null) {
			element.setAttribute(xml_attr_constructor, this.constructor.getClass().getName());
		}
	}
	...
	final void merge(SXElement element, SXMergeHelper helper) {
		...
		if (className != null && className.length() > 0) {
			this.constructor = helper.querier.find(ModelConstructor.class, className);
		}
	}
