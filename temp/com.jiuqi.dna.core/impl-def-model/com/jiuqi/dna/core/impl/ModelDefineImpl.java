...
import com.jiuqi.dna.core.def.model.ModelFieldDefine;
...
public final class ModelDefineImpl extends StructDefineImpl implements
	...
	public final static class HelperImpl implements Helper {
		...
		public final ModelDefineImpl newTempModelDeclare(SXElement template,
			...
			String className = template.getAttribute(ModelDefineImpl.xml_attr_moClass);
			modelDefine = new ModelDefineImpl(name, context.get(Class.class, className), null);
		...
		public void ensurePrepared(ModelDefine model, Context context) {
			...
			((ModelDefineImpl) model).ensurePrepared(ContextImpl.toContext(context), false);
		}
	}
	...
	public final static ModelDefineImpl tryParse(String fullName,
		...
		if (fullName.startsWith(type_name_prefix)) {
			...
			return (ModelDefineImpl) querier.find(ModelDefine.class, author, name);
		}
		return null;
	}
	...
	static {
		DataTypeUndigester.regUndigester(new DataTypeUndigester(TypeCodeSet.MODEL_H) {
			@Override
	...
	final void checkModelServiceMO(ModelInvokee modelInvokee) {
		...
		if (invokeeMO != this.soClass && !invokeeMO.isAssignableFrom(this.soClass)) {
			throw new UnsupportedOperationException("模型定义[" + this.name + "]的实例类型与所要绑定的模型服务不匹配");
		}
	}
	...
	public final ModelPropertyDefineImpl newProperty(String name, DataType type) {
		ModelPropertyDefineImpl property = new ModelPropertyDefineImpl(this, name, type, None.class);
	...
	private final ModelConstraintDefineImpl internalNewConstraint(String name,
		...
		ModelConstraintDefineImpl constraint = new ModelConstraintDefineImpl(this, name, kind, messageFormat);
	...
	public final ModelConstructorDefineImpl newConstructor(String name,
			Class<?> aoClass) {
		ModelConstructorDefineImpl cnstruct = new ModelConstructorDefineImpl(this, name, aoClass);
	...
	public final ModelActionDefineImpl newAction(String name, Class<?> aoClass) {
		ModelActionDefineImpl action = new ModelActionDefineImpl(this, name, aoClass);
	...
	public final MappingQueryStatementImpl newQuery(String name) {
		MappingQueryStatementImpl query = new MappingQueryStatementImpl(name, this);
	...
	public final ModelReferenceDefineImpl newReference(String name,
			ModelDefine target) {
		ModelReferenceDefineImpl ref = new ModelReferenceDefineImpl(this, name, (ModelDefineImpl) target);
		this.refs.add(ref);
		return ref;
	}
	...
	public final ModelObjSourceDefineImpl newSource(String name,
			Class<?> aoClass) {
		ModelObjSourceDefineImpl source = new ModelObjSourceDefineImpl(this, name, aoClass);
	...
	void merge(SXElement element, SXMergeHelper helper) {
		...
		for (SXElement constructorE : element.getChildren(xml_element_constructors, ModelConstructorDefineImpl.xml_element_constructor)) {
			String name = constructorE.getAttribute(NamedDefineImpl.xml_attr_name, null);
			ModelConstructorDefineImpl c = this.constructors.find(name);
			if (c == null) {
				c = new ModelConstructorDefineImpl(this, name, ArgumentableImpl.getAOClass(helper, constructorE));
				this.constructors.add(c);
			}
			c.merge(constructorE, helper);
		}
		for (SXElement propertyE : element.getChildren(xml_element_propertys, ModelPropertyDefineImpl.xml_element_property)) {
			String name = propertyE.getAttribute(NamedDefineImpl.xml_attr_name, null);
			ModelPropertyDefineImpl p = this.properties.find(name);
			if (p == null) {
				p = new ModelPropertyDefineImpl(this, name, propertyE.getAsType(ModelPropertyDefineImpl.xml_attr_type, helper.querier), ArgumentableImpl.getAOClass(helper, propertyE));
				this.properties.add(p);
			}
			p.merge(propertyE, helper);
		}
		for (SXElement actionE : element.getChildren(xml_element_actions, ModelActionDefineImpl.xml_element_action)) {
			String name = actionE.getAttribute(NamedDefineImpl.xml_attr_name, null);
			ModelActionDefineImpl a = this.actions.find(name);
			if (a == null) {
				a = new ModelActionDefineImpl(this, name, ArgumentableImpl.getAOClass(helper, actionE));
				this.actions.add(a);
			}
			a.merge(actionE, helper);
		}
		for (SXElement constraintE : element.getChildren(xml_element_constraints, ModelConstraintDefineImpl.xml_element_constraint)) {
			String name = constraintE.getAttribute(NamedDefineImpl.xml_attr_name, null);
			ModelConstraintDefineImpl c = this.constraints.find(name);
			if (c == null) {
				InfoKind kind = constraintE.getEnum(InfoKind.class, InfoDefineImpl.xml_attr_kind, InfoKind.ERROR);
				c = new ModelConstraintDefineImpl(this, name, kind, "");
				this.constraints.add(c);
			}
			c.merge(constraintE, helper);
		}
		for (SXElement queryE : element.getChildren(xml_element_querys, MappingQueryStatementImpl.xml_tag)) {
			String name = queryE.getAttribute(NamedDefineImpl.xml_attr_name, null);
		...
		for (SXElement objSourceE : element.getChildren(xml_element_sources, ModelObjSourceDefineImpl.xml_element_modelobjsource)) {
			String name = objSourceE.getAttribute(NamedDefineImpl.xml_attr_name, null);
			ModelObjSourceDefineImpl s = this.sources.find(name);
			if (s == null) {
				s = new ModelObjSourceDefineImpl(this, name, ArgumentableImpl.getAOClass(helper, objSourceE));
				this.sources.add(s);
			}
			s.merge(objSourceE, helper);
		}

		for (SXElement nestedE : element.getChildren(xml_element_nesteds, xml_element_model)) {
			String name = nestedE.getAttribute(NamedDefineImpl.xml_attr_name, null);
			String moClassName = nestedE.getAttribute(xml_attr_moClass);
			ModelDefineImpl model = this.nesteds.find(name);
			if (model == null) {
				model = new ModelDefineImpl(name, helper.querier.get(Class.class, moClassName), null);
