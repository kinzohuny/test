...
final class InspectPointImpl extends MetaBase implements InspectPoint {
	...
	final static void delayLoadInspectPoint(ModelDefineImpl model,
		...
		helper.addDelayAction(model, new SXMergeDelayAction<ModelDefineImpl>() {
			public void doAction(ModelDefineImpl at, SXMergeHelper helper,
					SXElement atElement) {
				for (SXElement element = first; element != null; element = element.nextSibling(InspectPointImpl.xml_element_inspectpoint)) {
					String actionName = element.getAttribute(InspectPointImpl.xml_attr_action, null);
					...
					if (actionName != null) {
						...
						String name = element.getString(InspectPointImpl.xml_attr_constraint);
						constraint = at.constraints.get(name);
						action = null;
					}
					...
					InspectPointImpl p = new InspectPointImpl(action, constraint);
					container.add(p);
				}
			}
		});
	...
	final void inspect(ContextImpl<?, ?, ?> context, Object mo,
		...
		if (this.action != null) {
			this.action.internalExecute(context, mo, null, trigger, triggerAO, value, stage);
		} else {
			this.constraint.internalCheck(context, mo, trigger, triggerAO, value, stage);
		}
	}
