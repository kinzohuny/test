...
abstract class ModelInvokeDefineImpl extends ArgumentableImpl implements
	...
	final void callBeforeInspects(ContextImpl<?, ?, ?> context, Object mo,
		...
		if (this.beforeInspects != null) {
			for (int i = 0, c = this.beforeInspects.size(); i < c; i++) {
				...
				ipi.inspect(context, mo, this, ao, value, ModelInvokeStage.BEFORE);
			}
		}
	}
	...
	final void callAfterInspects(ContextImpl<?, ?, ?> context, Object mo,
		...
		if (this.afterInspects != null) {
			for (int i = 0, c = this.afterInspects.size(); i < c; i++) {
				...
				ipi.inspect(context, mo, this, ao, value, ModelInvokeStage.AFTER);
			}
		}
	}
	...
	final void callFinallyInspects(ContextImpl<?, ?, ?> context, Object mo,
		...
		if (this.finallyInspects != null) {
			for (int i = 0, c = this.finallyInspects.size(); i < c; i++) {
				...
				ipi.inspect(context, mo, this, ao, value, ModelInvokeStage.FINALLY);
			}
		}
	}
	...
	public void render(SXElement element) {
		...
		if (this.beforeInspects != null) {
			this.beforeInspects.renderInto(element, xml_element_beforeInspects, 0);
		}
		if (this.afterInspects != null) {
			this.afterInspects.renderInto(element, xml_element_afterInspects, 0);
		}
		if (this.finallyInspects != null) {
			this.finallyInspects.renderInto(element, xml_element_finallyInspects, 0);
		}
	}
	...
	void merge(SXElement element, SXMergeHelper helper) {
		...
		SXElement inspect = element.firstChild(xml_element_afterInspects, InspectPointImpl.xml_element_inspectpoint);
		if (inspect != null) {
			InspectPointImpl.delayLoadInspectPoint(this.owner, this.getAfterInspects(), inspect, helper);
		}
		inspect = element.firstChild(xml_element_beforeInspects, InspectPointImpl.xml_element_inspectpoint);
		if (inspect != null) {
			InspectPointImpl.delayLoadInspectPoint(this.owner, this.getBeforeInspects(), inspect, helper);
		}
		inspect = element.firstChild(xml_element_finallyInspects, InspectPointImpl.xml_element_inspectpoint);
		if (inspect != null) {
			InspectPointImpl.delayLoadInspectPoint(this.owner, this.getFinallyInspects(), inspect, helper);
		}
	}
