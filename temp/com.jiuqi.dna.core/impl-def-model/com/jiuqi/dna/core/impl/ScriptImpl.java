...
import com.jiuqi.dna.core.def.model.ScriptDeclare;
...
final class ScriptImpl implements ScriptDeclare {
	...
	final boolean executeScriptAsAction(ContextImpl<?, ?, ?> context,
		...
		ModelScriptContext msContext = context.getScriptContext(this.engine);
		msContext.executeAction(invoke, this.prepareData, mo, ao, trigger, triggerAO, value);
		return true;
	}
	...
	final <TMO> boolean executeScriptAsSource(ContextImpl<?, ?, ?> context,
		...
		msContext.executeSource(source, this.prepareData, ao, offset, count, mos, factory);
		return true;
	}
	...
	final boolean executeScriptAsChecker(ContextImpl<?, ?, ?> context,
		...
		ModelScriptContext msContext = context.getScriptContext(this.engine);
		msContext.executeChecker(constraint, this.prepareData, mo, trigger, triggerAO, value);
		return true;
	}
	...
	final void prepareAsGetter(ContextImpl<?, ?, ?> context,
		...
		if (this.hasScript()) {
			...
			ModelScriptContext<?> sc = context.tryGetScriptContext(this.language);
