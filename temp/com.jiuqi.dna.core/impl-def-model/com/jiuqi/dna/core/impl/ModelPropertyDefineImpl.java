...
import com.jiuqi.dna.core.def.model.ListPropertyValue;
...
import com.jiuqi.dna.core.def.model.ModelPropertyDeclare;
import com.jiuqi.dna.core.def.model.ModelPropertyDefine;
import com.jiuqi.dna.core.def.model.ModelReferenceDefine;
import com.jiuqi.dna.core.impl.ModelServiceBase.ModelPropertyAccessor;
...
final class ModelPropertyDefineImpl extends ModelInvokeDefineImpl implements
	...
	final void checkModelAccessor(ModelPropertyAccessor accessor,
		...
		if (accessInfo == this.getterInfo) {
			if (!accessor.canGet(this.type)) {
				throw new UnsupportedOperationException("模型属性访问器[" + accessor + "]支持的类型与属性定义的类型(" + this.type + ")不符");
			}
	...
	final void callChangedInspects(ContextImpl<?, ?, ?> context, Object mo) {
		if (this.changedInspacts != null) {
			for (int i = 0, c = this.changedInspacts.size(); i < c; i++) {
				...
				ipi.inspect(context, mo, this, null, null, ModelInvokeStage.CHANGED);
			}
		}
	}
	...
	public final void render(SXElement element) {
		...
		if (this.changedInspacts != null) {
			this.changedInspacts.renderInto(element, xml_element_inspactChangeds, 0);
		}
	}
	...
	final void merge(SXElement element, SXMergeHelper helper) {
		...
		this.isStateEffective = element.getBoolean(xml_attr_isStateEffective, this.isStateEffective);
		...
		final SXElement changed = element.firstChild(xml_element_inspactChangeds, InspectPointImpl.xml_element_inspectpoint);
		if (changed != null) {
			InspectPointImpl.delayLoadInspectPoint(this.owner, this.getChangedInspects(), changed, helper);
		}
	}
	...
	private final void checkGetAccess(ContextImpl<?, ?, ?> context) {
		if (this.getterInfo == null || !this.getterInfo.isValid()) {
			throw new UnsupportedOperationException("模型[" + this.owner.name + "].属性[" + this.name + "]不支持读访问");
		}
	...
	private final void checkSetAccess(ContextImpl<?, ?, ?> context) {
		if (this.setterInfo == null || !this.setterInfo.isValid()) {
			throw new UnsupportedOperationException("模型[" + this.owner.name + "].属性[" + this.name + "]不支持写访问");
		}
		if (this.accessor == null) {
			this.accessor = ModelPropAccessor.setterOf(this.type);
		}
	...
	public final ListPropertyValue getPropValueAsList(Context context, Object mo) {
		...
		return this.accessor.getPropValueAsList(c, this.getterInfo, mo, None.NONE);
	}
