...
import com.jiuqi.dna.core.def.model.ModelObjSourceDeclare;
...
import com.jiuqi.dna.core.impl.ModelServiceBase.ModelObjProvider;
...
final class ModelObjSourceDefineImpl extends ArgumentableImpl implements
	...
	final void internalSetProvider(
		...
		if (provider != null && provider != this.provider) {
			...
			if (provider.aoClass != None.class && !provider.aoClass.isAssignableFrom(this.getAOClass())) {
	...
	final <TMO> void internalFetchMOs(ContextImpl<?, ?, ?> context, Object ao,
		...
		if (this.script != null && !this.script.executeScriptAsSource(context, ao, mos, offset, count, moFactory, this) && this.mappingQueryRef != null) {
			ao = this.mappingQueryRef.args.tryConvert(ao);
			if (ao == null) {
				throw new IllegalArgumentException("参数不匹配");
			}
			DBCommandProxy proxy = context.prepareStatement(this.mappingQueryRef);
			try {
				// TODO cmd.executeQueryLimit
				ResultSet resultSet = proxy.command.executeQuery(ao);
				try {
					ResultSetReader.readEntities(moFactory, mos, this.mappingQueryRef, resultSet);
				} finally {
					resultSet.close();
				}
			...
			SpaceNode occorAtSave = this.provider.getService().updateContextSpace(context);
			try {
				if (moFactory == null) {
					moFactory = (ObjectBuilder<TMO>) this;
				}
				this.provider.provide(context, ao, this, mos, offset, count, moFactory);
			} catch (Throwable e) {
				throw Utils.tryThrowException(e);
			} finally {
	...
	final int internalMOCountOf(ContextImpl<?, ?, ?> context, Object ao) {
		...
		if (this.script != null && this.script.scriptCallable()) {
			if (this.moCoutnOfScript != null && this.moCoutnOfScript.scriptCallable()) {
				return this.moCoutnOfScript.executeScriptAsSourceCountOf(context, ao, this);
			} else {
				return -1;
			}
			...
			return context.rowCountOf(this.mappingQueryRef, (DynamicObject) ao);
		} else if (this.provider != null) {
			SpaceNode occorAtSave = this.provider.getService().updateContextSpace(context);
			try {
	...
	public <TMO> void fetchMOs(Context context, Object ao, List<TMO> mos,
		...
		this.internalFetchMOs(ContextImpl.toContext(context), ao, mos, offset, count, null);
	}

	public final <TMO> void fetchMOs(Context context, List<TMO> mos,
			int offset, int count) {
		this.internalFetchMOs(ContextImpl.toContext(context), None.NONE, mos, offset, count, null);
	}
	...
	public <TMO> void fetchMOs(Context context, Object ao, List<TMO> mos,
		...
		this.internalFetchMOs(ContextImpl.toContext(context), None.NONE, mos, offset, count, moFactory);
	}
	...
	public final int moCountOf(Context context) {
		return this.internalMOCountOf(ContextImpl.toContext(context), None.NONE);
	}
	...
	void merge(SXElement element, SXMergeHelper helper) {
		...
		scriptE = element.firstChild(ModelObjSourceDefineImpl.xml_element_script_countof);
		if (scriptE != null) {
			this.getScript().merge(scriptE, helper);
		}
		String handlerClassName = element.getAttribute(ModelObjSourceDefineImpl.xml_attr_provider, null);
		if (handlerClassName != null && handlerClassName.length() > 0) {
			this.provider = helper.querier.find(ModelObjProvider.class, handlerClassName);
		}
		String ormName = element.getAttribute(ModelObjSourceDefineImpl.xml_attr_orm_name, null);
	...
	public void render(SXElement element) {
		...
		if (this.moCoutnOfScript != null) {
			this.moCoutnOfScript.tryRender(element, ModelObjSourceDefineImpl.xml_element_script_countof);
		}
		if (this.provider != null) {
			element.setAttribute(ModelObjSourceDefineImpl.xml_attr_provider, this.provider.getClass().getName());
		}
