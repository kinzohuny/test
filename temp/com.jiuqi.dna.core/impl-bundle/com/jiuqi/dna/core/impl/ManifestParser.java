...
final class ManifestParser {
	...
	final static MetaBaseContainerImpl<ExportPackage> parseExportPackage(
		...
		for (int i = 0, l = expArr.length; i < l; i++) {
			...
			ExportPackage exp = new ExportPackage(parseName(expstr), parseVersion(expstr), bundleInfo);
			container.add(exp);
		}
		return container;
	}
	...
	final static MetaBaseContainerImpl<ImportPackage> parseImportPackage(
		...
		while (m.find()) {
			String find = m.group();
			container.add(new ImportPackage(parseName(find), parseVersionRegion(find)));
		}
		return container;
	}
	 ...
	 * 匹配版本或者版本区间的正则表达式
	 */
	private final static Pattern pVersion = Pattern.compile("[1-9]+(\\.\\d+){2}(\\..*)?|[\\[\\(].*[\\]\\)]");
	/**
	 * 匹配一个含有版本区间的引入包或者依赖插件的正则表达式
	 */
	private final static Pattern pVersionRegionIn = Pattern.compile("[a-z].*?\\D,");
	...
	final static VersionRegion parseVersionRegion(String vrstr) {
		...
		if (j > 0) {
			...
			return new VersionRegion(Integer.parseInt(lvs[0]), Integer.parseInt(lvs[1]), Integer.parseInt(lvs[2]), Integer.parseInt(hvs[0]), Integer.parseInt(hvs[1]), Integer.parseInt(hvs[2]), includeL, includeH);
		} else {
	...
	final static MetaBaseContainerImpl<RequiredBundle> parseRequiredBundle(
		...
		while (m.find()) {
			String find = m.group();
			container.add(new RequiredBundle(parseName(find), parseVersionRegion(find)));
		}
		return container;
	}
	...
	final static Version parseVersion(String versionstr) {
		...
		return new Version(Integer.parseInt(versionArr[0]), Integer.parseInt(versionArr[1]), Integer.parseInt(versionArr[2]), qualifier);
	}
