...
final class ClusterConfig {
	...
	void connected(NetNodeInfo info) {
		...
		ConsoleLog.init("Cluster-Slave[No.%s of %s] connected.", index + 1, this.slaves.length);
	}
	...
	void broken(NetNodeInfo info) {
		...
		this.connectedBitmap &= (~(1 << info.index));
		ConsoleLog.init("Cluster-Slave[No.%s of %s] aborted.", index + 1, this.slaves.length);
	}
	...
	void succeeded(NetNodeInfo info) {
		...
		ConsoleLog.init("Cluster-Slave[No.%s of %s] succeeded.", index + 1, this.slaves.length);
	}
	...
	NetNodeInfo[] otherSlaves(NetNodeInfo exclusive) {
		...
		System.arraycopy(this.slaves, index + 1, otherSlaves, index, this.slaves.length - index - 1);
		return otherSlaves;
	}
