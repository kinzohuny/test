...
import com.jiuqi.dna.core.impl.DataWorker.WorkStatus;
...
class NetConnection extends NetManagerBased implements ByteBufferSender {
	...
	final void broken(final Throwable exception) {
		...
		ConsoleLog.info("与主机[%s:%s]的连接断开了。", this.remoteNodeInfo.getAddress().getHostAddress(), this.remoteNodeInfo.getPort());
		// help test
		// if (exception != null) {
	...
	final StructAdapterSet getStructAdapterSet() {
		if (this.structAdapterSet == null) {
			...
			if (c != null) {
				...
				this.structAdapterSet = new StructAdapterSet(this.netManager.application);
			}
		}
	...
	final StructDefineProvider structDefineProvider = new StructDefineProvider() {
		public StructAdapter getStructDefine(StructSummary structSummary)
			...
			try {
				RemoteQueryStubImpl stub = NetConnection.this.postRequest(new RemoteStructQuery(structSummary));
				stub.syncWork();
				...
				throw new StructDefineNotFoundException(structSummary.defineName, e);
			}
		}
	};
	...
	final void nofityForceCancelHandler(int requestId) {
		if (this.state == State.READY) {
			...
			putDataPacketHeaderData(buf, PacketCode.FORCE_CANCEL_HANDLER, 0, requestId, 0);
			buf.flip();
			this.toSend(wrapper);
		}
	}
	...
	final void nofityForceCancelStub(int requestId, Throwable exception) {
		if (this.state == State.READY) {
			try {
				RIUtil.send(requestId, this, new RemoteForceCancelStub(exception));
			} catch (Throwable ignore) {
				try {
					RIUtil.send(requestId, this, new RemoteForceCancelStub(null));
	 ...
	 */
	final DataPacket header = new DataPacket(this, (byte) -1, -1, DATAGRAM_HEADER_LENGTH);
	/**
	 * Only for this class and sub-classes.
	 */
	DataPacket packet;
	...
	WorkStatus receive(ByteBuffer buf) throws Exception {
		...
		if (size < 0) {
			...
			while (size > 0) {
				...
				if (this.header.isFull()) {
					if (this.packet == null) {
						this.packet = new DataPacket(this, this.header.get(DATAGRAM_HEADER_CODE_POS), this.header.getInt(this.remoteEndian, DATAGRAM_HEADER_REQUESTID_POS), this.header.getInt(this.remoteEndian, DATAGRAM_HEADER_LENGTH_POS));
					}
	...
	final void handOut(DataPacket dataPacket) {
		...
		Assertion.ASSERT(code != null, "警告！ - 数据包请求代码错误，原因可能是数据包接收或拆解出现错误。请求代码：" + dataPacket.code);
	...
	final void handOutRequest(DataPacket dataPacket) {
		...
		if (this.failureCominIDs.contains(dataPacket.requestId)) {
			...
			if (handler == null) {
				handler = new RemoteRequestHandler(this, dataPacket.requestId, false);
				this.comins.put(handler.id, handler);
				this.netManager.application.overlappedManager.startWork(handler);
			}
			handler.receiveRequestData(dataPacket);
		}
	}

	final void handStructRequest(DataPacket dataPacket) {
		Assertion.ASSERT(dataPacket.connection == this);
		RemoteRequestHandler handler = this.comins.get(dataPacket.requestId);
		if (this.failureCominIDs.contains(dataPacket.requestId)) {
			if (dataPacket.capacity() == 0) // 结束包
			...
			if (handler == null) {
				handler = new RemoteRequestHandler(this, dataPacket.requestId, true);
				this.comins.put(handler.id, handler);
				this.netManager.application.overlappedManager.startWork(handler);
			}
			handler.receiveRequestData(dataPacket);
