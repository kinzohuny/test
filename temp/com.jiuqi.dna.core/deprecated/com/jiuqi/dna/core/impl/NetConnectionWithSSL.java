...
final class NetConnectionWithSSL extends NetConnection {
	...
	NetConnectionWithSSL(NetManager netManager, NetNodeInfo remoteNodeInfo,
			Endianness remoteByteOrder, GUID remoteServerId,
			int remoteNodeInfoId, SocketChannel socketChannel,
			SSLEngine sslEngine) throws IOException {
		this(netManager, remoteNodeInfo, remoteByteOrder, remoteServerId, socketChannel, sslEngine);
	}
	...
	NetConnectionWithSSL(NetManager netManager, NetNodeInfo remoteNodeInfo,
			...
			throws IOException {
		super(netManager, remoteNodeInfo, remoteByteOrder, remoteServerId, socketChannel);
		if (sslEngine == null) {
			...
			while (this.toSend.size() > 0) {
				...
				do {
					...
					if (sslResult.getStatus() != Status.OK) {
						throw new IllegalStateException(sslResult.getStatus().toString());
					}
					do {
						if (this.channel.write(this.sslOut) == 0) {
							if (!src.hasRemaining()) {
