...
abstract class Endianness {
	...
	private static final long BYTES_BASE_OFFSET = UNSAFE.arrayBaseOffset(byte[].class);
	...
	static Endianness valueOf(final ByteOrder byteOrder) {
		...
		Endianness ed = byteOrder == ByteOrder.BIG_ENDIAN ? Endianness.BIG_ENDIAN : Endianness.LITTLE_ENDIAN;
		return ed.tryUnsafe();
	}
	...
	static final Endianness LITTLE_ENDIAN = new Endianness() {
		...
		@Override
		public double getDouble(byte[] b, int off) {
			long j = ((b[off + 0] & 0xFFL) << 0) + ((b[off + 1] & 0xFFL) << 8) + ((b[off + 2] & 0xFFL) << 16) + ((b[off + 3] & 0xFFL) << 24) + ((b[off + 4] & 0xFFL) << 32) + ((b[off + 5] & 0xFFL) << 40) + ((b[off + 6] & 0xFFL) << 48) + ((b[off + 7] & 0xFFL) << 56);
			return Double.longBitsToDouble(j);
		}

		@Override
		public float getFloat(byte[] b, int off) {
			int i = ((b[off + 0] & 0xFF) << 0) + ((b[off + 1] & 0xFF) << 8) + ((b[off + 2] & 0xFF) << 16) + ((b[off + 3] & 0xFF) << 24);
			return Float.intBitsToFloat(i);
		}
		...
		@Override
		public long getLong(byte[] b, int off) {
			return ((b[off + 0] & 0xFFL) << 0) + ((b[off + 1] & 0xFFL) << 8) + ((b[off + 2] & 0xFFL) << 16) + ((b[off + 3] & 0xFFL) << 24) + ((b[off + 4] & 0xFFL) << 32) + ((b[off + 5] & 0xFFL) << 40) + ((b[off + 6] & 0xFFL) << 48) + ((b[off + 7] & 0xFFL) << 56);
		}
	...
	static {
		...
		UNSAFE.putInt(a, (long) UNSAFE.arrayBaseOffset(byte[].class), 0x08040201);
		LOCAL_ENDIAN = a[0] == 0x01 ? Endianness.LITTLE_ENDIAN : Endianness.BIG_ENDIAN;
	}
}
