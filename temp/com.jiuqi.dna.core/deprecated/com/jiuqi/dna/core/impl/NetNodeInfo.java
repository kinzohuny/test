...
final class NetNodeInfo extends NetManagerBased {
	...
	synchronized final boolean tryBindConnection(NetConnection netConnection) {
		...
		if (netConnection.state() != State.READY && netConnection.state() != State.CONNECTED) {
			throw new IllegalStateException("要绑定的连接的状态无效：" + netConnection.state());
		}
		if (netConnection.remoteNodeInfo != this) {
			throw new IllegalArgumentException("无效绑定");
		}
		if (this.netConnection != null && (this.netConnection.state() == State.CONNECTED || this.netConnection.state() == State.READY)) {
			return false;
		}
		InetAddress remote = netConnection.getRemoteAddress();
		ConsoleLog.debugInfo("NodeInfoAddress(original:%s, new:%s)", this.address, remote);
		this.address = remote;
	...
	private SocketChannel openNewSocketChannel() throws IOException {
		SocketChannel channel = SocketChannel.open(new InetSocketAddress(this.getAddress(), this.port));
	...
	private final NetConnection openConnection() throws IOException {
		...
		final byte[] rsi_response_signal_data = NetManager.getRsiResponseSignalData();
		...
		this.secure = false;
		return new NetConnection(this.netManager, this, remote, remoteId, channel);
	}
	...
	private final NetConnection openConnectionWithSSL() throws IOException {
			...
			handshake: while (true) {
				...
				case NEED_UNWRAP:
					...
					do {
						...
						hsStatus = sslResult.getHandshakeStatus();
					} while (sslPacket.hasRemaining() && hsStatus == HandshakeStatus.NEED_UNWRAP);
				...
				case NEED_TASK:
					...
					break;
				default:
					throw new IllegalStateException("unhandled TLS/SSL handshake status: " + sslResult.getHandshakeStatus());
				}
			}
		}
		...
		this.secure = true;
		return new NetConnectionWithSSL(this.netManager, this, remote, remoteId, channel, sslEngine);
	}
