...
import com.jiuqi.dna.core.spi.http.ConstantsForDnaRsi;
...
final class NetManager implements ByteBufferManager {
		...
		if (this.locals == null) {
			synchronized (this) {
				if (this.locals == null) {
					...
					try {
						InetAddress[] temp = InetAddress.getAllByName(InetAddress.getLocalHost().getHostName());
	...
	NetManager(ApplicationImpl application) {
		...
		this.ID = application.netNodeManager.thisCluster.clusterID;
	...
	final void config(SXElement clusterInfo) {
		if (clusterInfo != null) {
			...
			if (host != null) {
				int port = clusterInfo.getInt(xml_a_cluster_masterport, RIUtil.DEFAULT_PORT);
				try {
					this.clusterConfig = new ClusterConfig(this.nodeInfos.ensureGet(host, port, true));
				} catch (UnknownHostException e) {
				...
				if (slave != null) {
					...
					do {
						...
						port = slave.getInt(xml_a_cluster_slave_port, RIUtil.DEFAULT_PORT);
						try {
							temp.add(this.nodeInfos.ensureGet(host, port, false));
						} catch (UnknownHostException e) {
							throw Utils.tryThrowException(e);
						}
						slave = slave.nextSibling(xml_e_cluster_slave);
					} while (slave != null);
					this.clusterConfig = new ClusterConfig(temp.toArray(new NetNodeInfo[temp.size()]));
				} else {
					this.clusterConfig = null;
				}
			}
	...
	private final void startMaster() {
		...
		synchronized (this.clusterConfig) {
			...
			} while (System.currentTimeMillis() - startat < timeout && !this.clusterConfig.isStarted());
		}
	...
	static final byte[] getRsiResponseSignalData() {
		if (rsi_response_signal_data == null) {
			rsi_response_signal_data = ConstantsForDnaRsi.ascii2bytes(ConstantsForDnaRsi.RSI_RESPONSE_SIGNAL);
		}
	...
	private final void resolveClusterInfo(NetConnection nc) throws IOException,
		...
		if (inCluster) {
			if (startToMaster) {
				...
				// node's index
				Endianness.LOCAL_ENDIAN.putInt(buf, 0, this.application.netNodeManager.thisCluster.thisClusterNodeIndex);
				out.write(buf, 0, 4);
			}
		}
		out.flush();
	...
	private final void acceptSlaveNode(NetConnection netConnection,
		...
		if (otherSlaves.length > 0) {
			SOSerializer sos = new SOSerializer(new StreamBasedDataSerializer(out, Endianness.LOCAL_ENDIAN));
		...
		if (code == 1) {
			...
			try {
				netConnection.broken(new IllegalStateException("失败的验证码：" + code));
	...
	private final void receiveClusterConfig(NetConnection netConnection,
		...
		int clusterWholeMask = IOHelper.readInt(in, netConnection.remoteEndian, buf);
		...
		if (len == -1) {
			...
			try {
				SODeserializer sod = new SODeserializer(netConnection.getStructAdapterSet(), new StreamBasedDataDeserializer(in, netConnection.remoteEndian));
		...
		if (flag) {
			...
			if (in.read() == 1) {
				...
				if (isJudge) {
					...
					if (in.read() == 0) {
						using = this.connections.get(IOHelper.readInt(in, nc.remoteEndian, null));
	...
	private static final void logConnectingEvent(InetAddress address, int port,
			boolean isClient) {
		ConsoleLog.info("建立了%s主机[%s:%s]的连接。", isClient ? "到" : "来自", address.getHostAddress(), port);
	}
	...
	private final NetNodeInfoStorage nodeInfos = new NetNodeInfoStorage(new NetNodeInfoCreator() {
		public NetNodeInfo create(InetAddress address, int port,
				boolean isMaster) {
			return NetNodeInfo.createNNI(NetManager.this, address, port, isMaster);
		}

		public NetNodeInfo create(String host, int port, boolean isMaster) {
			return NetNodeInfo.createNNI(NetManager.this, host, port, isMaster);
		}
	});
	...
	private static class NetNodeInfoStorage {
		...
		final NetNodeInfo tryPut(GUID serverId, NetNodeInfo nodeInfo) {
			synchronized (this.idNodes) {
				NetNodeInfo old = this.idNodes.get(serverId, nodeInfo.getPort());
