...
final class ByteBufferPool {
	...
	ByteBufferPool(int capacity, int bufSize) {
		if (capacity <= 0) {
			throw new IllegalArgumentException("给定的容量（capacity）值不合法：" + capacity);
		}
		if (bufSize <= 0) {
	...
	final synchronized ByteBufferWrapper get() {
		...
		if (!this.pool.empty()) {
			...
			buf = new ByteBufferWrapper(this, ByteBuffer.allocateDirect(this.bufSize));
			this.size++;
		}
	...
	private final synchronized void free(ByteBufferWrapper buf) {
		if (buf != null) {
			Assertion.ASSERT(!(buf instanceof ByteBufferWrapper.ReadOnlyByteBufferWrapper));
	...
	static class ByteBufferWrapper {
		...
		synchronized ByteBufferWrapper newSpecialViewer() {
			...
			ReadOnlyByteBufferWrapper viewer = new ReadOnlyByteBufferWrapper(this);
		...
		private static final class ReadOnlyByteBufferWrapper extends
			...
			ReadOnlyByteBufferWrapper(ByteBufferWrapper byteBufferWrapper) {
				...
				if (byteBufferWrapper instanceof ReadOnlyByteBufferWrapper) {
					throw new IllegalArgumentException("Unsupported wrapper type: " + byteBufferWrapper.getClass().getName());
				}
