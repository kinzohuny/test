...
abstract class RemoteRequestStubBase implements RemoteRequestStub,
	...
	public final void setRemoteException(ThrowableAdapter exception) {
		...
		this.exception = new RemoteException(this.remoteRequest, cni.getAddress(), cni.getPort(), exception);
	}
		...
		synchronized (this.resultPackets) {
			while (!this.resultPackets.finished()) {
				try {
					...
					ConsoleLog.debugInfo("远程请求线程[%s]在等待远程返回的处理结果传输完毕时被中断：%s", Thread.currentThread().getName(), e);
					return e;
				}
			}
		}
	...
	final void syncWork() {
		...
		// receive data
		try {
			RemoteReturn rr = (RemoteReturn) RIUtil.parse(this.netConnection, this.resultPackets);
			rr.setReturn(this);
