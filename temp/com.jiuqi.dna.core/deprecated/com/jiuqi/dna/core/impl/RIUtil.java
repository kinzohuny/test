...
final class RIUtil {
	...
	static final RemoteCommand parse(final NetConnection connection,
		...
		DataPacketDeserializer in = new DataPacketDeserializer(dataSrc, connection.remoteEndian);
		SODeserializer sod = new SODeserializer(connection.getStructAdapterSet(), in, connection.structDefineProvider);
		Object obj = sod.deserialize();
		if (obj instanceof RemoteCommand) {
			return (RemoteCommand) obj;
		} else {
			throw new UnknownObjectException("无法识别的数据：[" + obj.getClass() + "]" + obj);
		}
	}

	static final void send(final int requestId, final NetConnection connection,
			final RemoteCommand command) throws Throwable {
		ByteBufferSerializer bbs = new ByteBufferSerializer(connection, connection.netManager, command.getPacketCode(), requestId);
		try {
	...
	static final Thread startDaemon(Runnable runnable, String threadName) {
		if (threadName == null) {
			...
			Thread t = new Thread(runnable, "dna-ri-" + threadName);
			t.setDaemon(true);
			t.start();
			return t;
		}
