...
final class StructAdapterSet {
	...
	static StructAdapter newAdapter(StructSummary remoteStructSummary,
		...
		return new StructAdapter(remoteStructSummary, remoteFieldCount, saContext.findLocal(remoteStructSummary));
	}
	...
	private StructDefineImpl findLocal(StructSummary summary)
		...
		try {
			...
			// }
			final DataType odt = DataTypeBase.dataTypeOfJavaClassForOldSerial(this.getLocalClass(summary.defineName));
	...
	synchronized StructAdapter findAdapter(StructSummary summary) {
		...
		while (a != null) {
			if (a.remoteStructSummary == summary || a.remoteStructSummary.equalNameAndVUIDs(summary)) {
	...
	synchronized StructAdapter putAdapter(StructAdapter adapter) {
		...
		// add new
		this.ensureCapacity();
		index = UtilHelper.indexForObjectKey(adapter.remoteStructSummary, this.adapters.length);
	...
	private void ensureCapacity() {
		if (this.size >= this.adapters.length * loadFactor) {
			...
			for (int i = 0, len = this.adapters.length; i < len; i++) {
				...
				while (a != null) {
					temp = a.next;
					newIndex = UtilHelper.indexForObjectKey(a.remoteStructSummary, newSize);
