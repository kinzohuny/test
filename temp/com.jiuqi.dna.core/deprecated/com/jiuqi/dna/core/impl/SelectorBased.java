 ...
 * @version 1.0
 */
abstract class SelectorBased extends NetManagerBased implements Runnable {
	...
	public final void run() {
				...
				for (Iterator<SelectionKey> i = selected.iterator(); i.hasNext();) {
					...
					if (key.isValid()) {
						...
						if (c.state() == State.READY) {
							...
							key.cancel();
							c.broken(new IllegalStateException(c.state().toString()));
						}
					}
				}
			}
		...
		System.err.format("%1$tY-%1$tm-%1$td %1$tT FATAL - 通道状态监护线程中止：%2$s%n", new Date(), this.exception);
	}
}
