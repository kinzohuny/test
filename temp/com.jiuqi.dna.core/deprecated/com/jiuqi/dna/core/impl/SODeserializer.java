...
final class SODeserializer implements StructuredObjectDeserializer {
	...
	private static final HashMap<String, Class<?>> primClasses = new HashMap<String, Class<?>>(9, 1.0F);
	static {
	...
	private void readStreamHeader() throws StreamCorruptedException,
			IOException {
		if (this.in.readShort() != SerialStreamConstants.STREAM_MAGIC || this.in.readShort() != SerialStreamConstants.STREAM_VERSION) {
	...
	private static <T> T checkRead(SODeserializer sod, CheckedReader<T> reader)
		...
		try {
			...
			ClassNotFoundException ex = sod.handles.lookupException(sod.passHandle);
			if (ex != null) {
	...
	private Object readHandle() throws IOException {
		...
		this.passHandle = this.in.readInt() - SerialStreamConstants.baseWireHandle;
	...
	private Enum<?> readNewEnum() throws IOException {
		...
		if (enumClass != null) {
			try {
				...
				throw (IOException) new InvalidObjectException("enum constant " + name + " does not exist in " + enumClass).initCause(e);
			}
	...
	private final StructAdapter readNewStructDefine() throws IOException,
		...
		StructAdapter adapter = StructAdapterSet.newAdapter(structSummary, size, this.saContext);
		this.handles.setObject(handle, adapter);

		// 这里需要保证和写的过程严格对称。
		for (short i = 0; i < size; i++) {
			this.passHandle = handle;
			adapter.addNewFieldAdapter(this.readUTF8()/* name */, this.in.readBoolean()/* isStateField */, this.in.readBoolean()/* isReadOnly */, this.in.readBoolean()/* isKeepValid */, DataTypeHelper.readDataType(this.FIELD_TYPE_UNDIGESTER));
		}
	...
	private Object readOrdinaryObject() throws IOException,
		...
		if (adapter != null) {
			...
			this.handles.markException(handle, new ClassNotFoundException(summary.defineName));
		}
		this.passHandle = handle;
	...
	private static class HandleTable {
		...
		Object lookupObject(int handle) {
			return (handle != NULL_HANDLE && this.status[handle] != STATUS_EXCEPTION) ? this.entries[handle] : null;
		}
		...
		ClassNotFoundException lookupException(int handle) {
			return (handle != NULL_HANDLE && this.status[handle] == STATUS_EXCEPTION) ? (ClassNotFoundException) this.entries[handle] : null;
		}
	...
	static Object fromBinary(StructAdapterSet structAdapterContext,
		...
		SODeserializer sod = new SODeserializer(structAdapterContext, new StreamBasedDataDeserializer(in, endian));
