...
class DnaNetListener extends NetListener {
	...
	void acceptSocketChannel(SocketChannel socketChannel) {
		RIUtil.startDaemon(new NetRsiHttpHeaderHandler(socketChannel, this.netManager.ACCEPTOR), "connectionHandler");
	}
			...
			public void run() {
				...
				while (true) {
					if (Thread.interrupted()) {
						ConsoleLog.info("远程服务调用的连接监听程序［端口：%s］被要求关闭", DnaNetListener.this.port);
						ConsoleLog.info("远程服务调用的连接监听程序［端口：%s］开始执行关闭操作...", DnaNetListener.this.port);
						DnaNetListener.this.close();
						ConsoleLog.info("远程服务调用的连接监听程序［端口：%s］已经关闭", DnaNetListener.this.port);
						break;
					}
					...
					try {
						...
						ConsoleLog.debugError("远程服务调用的连接监听程序［端口：%s］在设置接收到的Socket为无延迟工作时出现异常：%s", DnaNetListener.this.port, e);
						continue;
					}
				}
			}
...
final class SecureDnaNetListener extends DnaNetListener {
	...
	void acceptSocketChannel(SocketChannel socketChannel) {
		...
		this.netManager.ACCEPTOR.handleDetackedSocketChannel(socketChannel, null);
	}
}
