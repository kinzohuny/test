...
class HandshakorWithSSL extends Handshakor {
	...
	private class SSLInputStream extends InputStream {
		final ByteBuffer sslPacket = (ByteBuffer) ByteBuffer.allocate(HandshakorWithSSL.this.sslEngine.getSession().getPacketBufferSize()).flip();
		final ByteBuffer msg = (ByteBuffer) ByteBuffer.allocate(HandshakorWithSSL.this.sslEngine.getSession().getPacketBufferSize()).flip();
		...
		public int read() throws IOException {
			...
			do {
				...
				if (this.sslPacket.hasRemaining()) {
					SSLEngineResult result = HandshakorWithSSL.this.sslEngine.unwrap(this.sslPacket, this.msg);
	...
	private class SSLOutputStream extends OutputStream {
		final ByteBuffer sslPacket = ByteBuffer.allocate(HandshakorWithSSL.this.sslEngine.getSession().getPacketBufferSize());
		final ByteBuffer msg = ByteBuffer.allocate(HandshakorWithSSL.this.sslEngine.getSession().getPacketBufferSize());
		...
		public void flush() throws IOException {
			if (this.msg.position() > 0) {
				...
				while (this.msg.hasRemaining()) {
					this.sslPacket.clear();
					HandshakorWithSSL.this.sslEngine.wrap(this.msg, this.sslPacket);
					this.sslPacket.flip();
					do {
						HandshakorWithSSL.this.socketChannel.write(this.sslPacket);
					} while (this.sslPacket.hasRemaining());
				}
				this.msg.clear();
			}
