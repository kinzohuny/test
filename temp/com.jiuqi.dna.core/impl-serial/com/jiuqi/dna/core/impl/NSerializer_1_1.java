...
public class NSerializer_1_1 extends NSerializer implements NSerializeBase_1_1 {
	...
	public static final NSerializerFactory FACTORY = new NSerializerFactory(SERIALIZE_VERSION) {
		...
		public final NSerializer newNSerializer() {
			return new NSerializer_1_1();
		}

	};

	public NSerializer_1_1() {
	...
	public short getVersion() {
		return NSerializeBase_1_1.SERIALIZE_VERSION;
	}

	@Override
	public final boolean serializeStart(final Object object,
			final DataOutputFragment fragment) {
		return this.serializeStart(object, fragment, false);
	}
	...
	final boolean writeClassData(final Class<?> value) {
		final DataType dataType = DataTypeBase.dataTypeOfJavaClass(value);
		final GUID typeID = dataType.getID();
		final int IDIndex = this.objectIndexMap.tryGetIndex(typeID);
		if (IDIndex < 0) {
			if (this.fully) {
				final boolean fail = (dataType instanceof ArrayDataTypeBase) ? this.writeClassOfData(typeID, value, HEAD_WITHCLASSNAME_ARRAYCLASS, 0) : this.writeClassOfData(typeID, value, HEAD_WITHCLASSNAME_CLASS, 0);
				if (fail) {
					return SERIALIZE_FAIL;
				}
			} else {
				if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID) {
					...
					this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_CLASS);
			...
			if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER) {
				...
				this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_CLASS);
		...
		if (declared) {
			...
			if (IDIndex < 0) {
				if (this.fully) {
					final boolean fail;
					if (isSmallEnum) {
						fail = this.writeClassOfData(typeID, value.getClass(), HEAD_WITHCLASSNAME_SMALLENUM, SIZE_BYTE);
					} else {
						fail = this.writeClassOfData(typeID, value.getClass(), HEAD_WITHCLASSNAME_LARGEENUM, SIZE_POINTER);
					}
					if (fail) {
						return SERIALIZE_FAIL;
					}
					...
					if (isSmallEnum) {
						if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID + SIZE_BYTE) {
							...
							this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_SMALLENUM);
						}
					} else {
						if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID + SIZE_INT) {
							return SERIALIZE_FAIL;
						} else {
							this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_LARGEENUM);
						}
					}
					this.forceWriteGUIDValue(typeID);
				}
				...
				if (isSmallEnum) {
					if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER + SIZE_BYTE) {
						...
						this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_SMALLENUM);
					}
				} else {
					if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER + SIZE_INT) {
						return SERIALIZE_FAIL;
					} else {
						this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_LARGEENUM);
					}
				}
				this.dataOutputFragment.writeInt(IDIndex);
			}
	...
	final boolean writeObjectArrayData(final ObjectArrayDataType arrayType,
		...
		if (IDIndex < 0) {
			if (this.fully) {
				final Class<?> rootComponentClass = arrayType.getRootComponentClass();
				final boolean fail;
				if (isEmptyArray) {
					fail = this.writeClassOfData(typeID, rootComponentClass, HEAD_WITHCLASSNAME_EMPTYOBJECTARRAY, 0);
				} else {
					fail = this.writeClassOfData(typeID, rootComponentClass, HEAD_WITHCLASSNAME_OBJECTARRAY, SIZE_INT);
				...
				if (isEmptyArray) {
					if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID) {
						...
						this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_EMPTYOBJECTARRAY);
						this.forceWriteGUIDValue(typeID);
					}
				} else {
					if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID + SIZE_INT) {
						...
						this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_OBJECTARRAY);
						this.forceWriteGUIDValue(typeID);
						this.dataOutputFragment.writeInt(value.length);
					}
				}
			...
			if (isEmptyArray) {
				if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER) {
					...
					this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_EMPTYOBJECTARRAY);
					this.dataOutputFragment.writeInt(IDIndex);
				}
			} else {
				if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER + SIZE_INT) {
					...
					this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_OBJECTARRAY);
					this.dataOutputFragment.writeInt(IDIndex);
					this.dataOutputFragment.writeInt(value.length);
				}
			}
	...
	final boolean writeStructData(final Object value,
		...
		if (declared) {
			...
			final int IDIndex = this.objectIndexMap.tryGetIndex(typeID);
			if (IDIndex < 0) {
				if (this.fully && (javaClass = structDefine.getJavaClass()) != null) {
					if (this.writeClassOfData(typeID, javaClass, HEAD_WITHCLASSNAME_STRUCT, 0)) {
						return SERIALIZE_FAIL;
					}
				} else {
					if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID) {
						...
						this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_STRUCT);
						this.forceWriteGUIDValue(typeID);
					}
				}
			} else {
				if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER) {
					...
					this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_STRUCT);
					this.dataOutputFragment.writeInt(IDIndex);
				}
			}
		}
	...
	final boolean writeCustomSerializeDataObject(final Object value,
		...
		if (IDIndex < 0) {
			if (this.fully) {
				if (this.writeClassOfData(typeID, value.getClass(), HEAD_WITHCLASSNAME_CUSTOM, SIZE_SHORT)) {
					return SERIALIZE_FAIL;
				}
			} else {
				if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID + SIZE_SHORT) {
					...
					this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_CUSTOM);
					this.forceWriteGUIDValue(typeID);
				}
			}
		} else {
			if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER + SIZE_SHORT) {
				...
				this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_CUSTOM);
				this.dataOutputFragment.writeInt(IDIndex);
			}
		}
	...
	private final boolean writePointer(final int pointer) {
		if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER) {
			return SERIALIZE_FAIL;
		} else {
	...
	private final boolean writeNullObject() {
		switch (this.CState) {
		case NULL:
	...
	private final void forceWriteGUIDValue(final GUID value) {
		this.dataOutputFragment.writeLong(value.getMostSigBits());
		this.dataOutputFragment.writeLong(value.getLeastSigBits());
		this.objectIndexMap.tryPutObject(value);
	}

	private final boolean writeVarLengthObject(final FixLengthObject empty,
	...
	private final boolean writeDataType(final DataType type) {
		...
		final int IDIndex = this.objectIndexMap.tryGetIndex(typeID);
		if (IDIndex < 0) {
			if (this.fully && (javaClass = type.getJavaClass()) != null) {
				final boolean fail = (type instanceof ArrayDataTypeBase) ? this.writeClassOfData(typeID, javaClass, HEAD_WITHCLASSNAME_ARRAYTYPE, 0) : this.writeClassOfData(typeID, javaClass, HEAD_WITHCLASSNAME_TYPE, 0);
				if (fail) {
					return SERIALIZE_FAIL;
				}
			} else {
				if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID) {
					...
					this.dataOutputFragment.writeByte(HEAD_WITHTYPEID_TYPE);
					this.forceWriteGUIDValue(typeID);
				}
			}
		} else {
			if (this.dataOutputFragment.remain() < SIZE_HEAD + SIZE_POINTER) {
				...
				this.dataOutputFragment.writeByte(HEAD_WITHTYPEIDPOINTER_TYPE);
				this.dataOutputFragment.writeInt(IDIndex);
			}
		}
	...
	private final boolean writeClassOfData(final GUID typeID,
			final Class<?> rootClazz, final byte withClassNameHeadOfData,
			final int otherDataLen) {
		final DataOutputFragment dataOutputFragment = this.dataOutputFragment;
		final String className = rootClazz.getName();
		final int classNameLen = className.length();
		if (dataOutputFragment.remain() < SIZE_HEAD + SIZE_GUID + SIZE_INT + classNameLen * SIZE_CHAR + otherDataLen) {
			return true;
		} else {
			dataOutputFragment.writeByte(withClassNameHeadOfData);
			dataOutputFragment.writeLong(typeID.getMostSigBits());
			dataOutputFragment.writeLong(typeID.getLeastSigBits());
			dataOutputFragment.writeInt(classNameLen);
			for (int index = 0, endIndex = classNameLen; index < endIndex; index++) {
				dataOutputFragment.writeChar(className.charAt(index));
			}
	...
	private final boolean tryFinishWriteC() {
		if (this.CState != ContinuousState.NONE) {
			if (this.dataOutputFragment.remain() < SIZE_BYTE) {
				return SERIALIZE_FAIL;
			} else {
