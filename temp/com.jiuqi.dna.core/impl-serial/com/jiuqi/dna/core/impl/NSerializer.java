...
public abstract class NSerializer {
	 ...
	 * 必须按照版本号从大到小的顺序排放
	 */
	private static NSerializerFactory[] factorys = new NSerializerFactory[] { NSerializer_1_1.FACTORY, NSerializer_1_0.FACTORY };
	...
	public static NSerializerFactory getRemoteCompatibleFactory(
		...
		throw new IllegalArgumentException("无法取得与远程节点兼容的序列化器，远程序列化协议版本过低: version(" + remoteVersion + ")");
	}
	 ...
	 * 该方法仅供内部调用和回调使用
	 */
	abstract boolean writeObjectArrayElement(final Object value);

	/**
	 * 该方法仅供内部调用和回调使用
	 */
	abstract boolean writeCustomData(final Object value);

	static final RuntimeException serializeException() {
