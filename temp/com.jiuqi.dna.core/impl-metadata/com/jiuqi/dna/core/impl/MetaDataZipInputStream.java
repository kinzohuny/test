...
import com.jiuqi.dna.core.spi.metadata.MetaDataEntry;
...
public final class MetaDataZipInputStream extends ZipInputStream {
	...
	private final static class MetaDataByteArrayInputStream extends
		...
		private final int read32(int pos) {
			return (this.buf[pos] & 0xff) | ((this.buf[pos + 1] & 0xff) << 8) | ((this.buf[pos + 2] & 0xff) << 16) | ((this.buf[pos + 3] & 0xff) << 24);
		}
		...
		private int getFirstCENOffset() {
			...
			for (int pos = this.count - ENDHDR + 3, end = Math.max(3, pos - 0xFFFF); pos >= end; pos--) {
				if (this.buf[pos] == s1 && this.buf[--pos] == s2 && this.buf[--pos] == s3 && this.buf[--pos] == s4) {
					final int offset = this.read32(pos + ENDOFF);
					if (0 < offset && offset < pos && this.read32(offset) == CENSIG) {
						return offset;
					}
				}
			}
		...
		private int readCEN(MetaDataEntryImpl root, int pos) {
			...
			final long version;
			if (extral == 8) {
				version = (this.read32(pos) & 0xffffffffl) | ((long) (this.read32(pos + 4)) << 32);
			} else {
				version = 0;
			}
			pos += extral;
	...
	public MetaDataZipInputStream(byte[] buf) {
		super(dummy);
		final MetaDataByteArrayInputStream stream = new MetaDataByteArrayInputStream(buf);
