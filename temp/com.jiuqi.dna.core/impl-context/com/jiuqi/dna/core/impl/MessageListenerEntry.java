...
import com.jiuqi.dna.core.situation.MessageDirection;
	...
	final <TMSG> MessageListenerEntry<TMSG> add(Class<TMSG> msgClass,
		...
		for (;;) {
			if (t.messageClass == msgClass) {
				...
				MessageListenerEntry mle = new MessageListenerEntry<TMSG>(msgClass, listener, this.situation);
				t.nextListener = mle;
				return mle;
			}
			if (t.nextMessage == null) {
				MessageListenerEntry entry = new MessageListenerEntry<TMSG>(msgClass, listener, this.situation);
				t.nextMessage = entry;
				return entry;
			}
			t = t.nextMessage;
			...
			do {
				...
				handleMessage: {
					...
					if (transmitter.getDirection() == MessageDirection.BRODCAST) {
						if (entry.disableListenBroadcast) {
							break handleMessage;
						}
					} else if (entry.disableListenBubble) {
						break handleMessage;
					}
					if (entry.listenSubMessage) {
						if (sameClass == 0) {
							if (entry.messageClass.isAssignableFrom(messageClass)) {
								sameClass = -1;
							} else {
								// 类型完全不同，退出该组
					...
					if (listener != null) {
						...
						listener.onMessage(transmitter.context, transmitter.message, transmitter);
