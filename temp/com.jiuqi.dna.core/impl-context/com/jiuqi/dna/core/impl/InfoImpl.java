...
class InfoImpl implements Info {
	...
	final void setParam(int index, Object value) {
		...
		this.params[index] = paramDefine.type.convert(value);
	}
		...
		if (ps > 0) {
			...
			if (ps > 1) {
				...
				if (ps > 2) {
					...
					int i = 3;
					if (others != null) {
						for (int othersL = others.length; i < ps && otherOffset < othersL; i++, otherOffset++) {
							params[i] = pas.get(i).convertWithDefault(others[otherOffset]);
						}
					}
					while (i < ps) {// 填充默认值
	...
	public final Object getParam(int index) {
		return 0 <= index && index < this.params.length ? this.params[index] : null;
	}
