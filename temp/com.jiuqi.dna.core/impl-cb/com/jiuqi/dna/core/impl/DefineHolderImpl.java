...
import com.jiuqi.dna.core.cb.DefineHolder;
...
class DefineHolderImpl extends ObjectQuerierImpl implements DefineHolder {
	...
	public final void putDefine(MetaElementType type, Reader reader,
		...
		case DELETE:
		case UPDATE:
			NDmlDeclare statement = DNASql.parseNStatement(reader, NDmlDeclare.class, this);
			this.put(type, statement.name.value, statement, pkgname, clzname);
			break;
		case TABLE:
			NAbstractTableDeclare raw = DNASql.parseNStatement(reader, NAbstractTableDeclare.class, this);
	...
	public final void putTableDefine(InputStream in, String pkgname,
		...
		try {
			SXElement element = this.toXml.build(in).firstChild(TableDefineImpl.table_tag);
				...
				try {
					while (rs.next()) {
						...
						if (type == MetaElementType.TABLE) {
							SXElement element = this.toXml.build(rs.getCharacterStream(3)).firstChild(TableDefineImpl.table_tag);
							TableDefineImpl table = new TableDefineImpl(name, null);
							table.id = id;
							try {
								table.mergeDelayRelation(element, this.helper);
								this.put(MetaElementType.TABLE, name, table, null, null);
							} catch (Throwable e) {
	...
	static final class TwoKeyEntry {
		...
		private TwoKeyEntry(MetaElementType type, String name) {
		...
		private boolean equals(MetaElementType type, String name) {
			return this.type == type && (this.name == name || this.name.equals(name));
		}
	...
	final void put(MetaElementType type, String name, Object define,
		...
		if (table == null) {
			...
			TwoKeyEntry e = new TwoKeyEntry(type, name);
			e.define = define;
			e.pkgname = pkgname;
			e.clzname = clzname;
		...
		TwoKeyEntry ne = new TwoKeyEntry(type, name);
		ne.define = define;
		ne.pkgname = pkgname;
		ne.clzname = clzname;
		ne.next = table[index];
	...
	private static final TableDefineImpl getTableOnly(TwoKeyEntry e) {
		return e.define instanceof TableDefineImpl ? (TableDefineImpl) e.define : null;
	}
