...
import com.jiuqi.dna.core.cb.DeclaratorBuilder;
import com.jiuqi.dna.core.cb.DefineProvider;
...
public final class DeclaratorBuilderImpl extends DefineHolderImpl implements
	...
	public final void build(Appendable out, MetaElementType type, String name,
		...
		try {
			this.provider = coalesce(provider, this.provider);
			...
			if (entry == null) {
				throw new MissingDefineException("不存在类型为[" + type + "],名称为[" + name + "]的元数据定义.");
			}
			switch (type) {
			case TABLE:
				...
				if (define instanceof TableDeclareStub) {
					...
					this.buildTableDeclarator(builder, nTable.getTable(), entry.pkgname, entry.clzname);
				} else if (define instanceof NAbstractTableDeclare) {
					NAbstractTableDeclare table = (NAbstractTableDeclare) define;
					this.buildAbstractTableDeclarator(builder, table, entry.pkgname, entry.clzname);
					...
					this.buildTableDeclarator(builder, table, entry.pkgname, entry.clzname);
				} else {
			...
			case UPDATE:
				...
				buildModifyStatementDeclarator(builder, statement, entry.pkgname, entry.clzname);
				break;
			case QUERY:
				NQueryDeclare query = (NQueryDeclare) entry.define;
				buildQueryStatementDeclarator(builder, query, entry.pkgname, entry.clzname);
				break;
			case ORM:
				NOrmDeclare orm = (NOrmDeclare) entry.define;
				buildORMDeclarator(builder, orm, entry.pkgname, entry.clzname);
				break;
			default:
				throw new UnsupportedOperationException();
			}
	...
	private final void buildTableDeclarator(CodeBuilder builder,
		...
		if (relationed) {
			...
			for (TableRelationDefineImpl relation : table.relations) {
				TwoKeyEntry e = this.get(MetaElementType.TABLE, relation.target.name);
				if (e == null) {
					throw new MissingDefineException("表关系[" + relation.name + "]的目标表[" + relation.target.name + "]不能存在.");
				}
		...
		// *********** members ***********
		declareStringWithAssign(builder, PUBLIC | STATIC | FINAL, TABLE_NAME, table.name);
		builder.appendLine();
		if (fielded) {
			declareDefines(builder, PUBLIC | FINAL, TableFieldDefine.class, table.fields, 2);
			builder.appendLine();
			for (TableFieldDefineImpl field : table.fields) {
				if (field == table.f_recid || field == table.f_recver) {
					continue;
				}
				declareStringWithAssign(builder, PUBLIC | STATIC | FINAL, declareNameOfFN(field), field.name);
			}
			builder.appendLine();
		}
		if (relationed) {
			declareDefines(builder, PRIVATE, TableRelationDeclare.class, table.relations, 0);
			builder.appendLine();
		}
		if (hierarchied) {
			declareDefines(builder, PUBLIC | FINAL, HierarchyDefine.class, table.hierarchies, 0);
			builder.appendLine();
		}
		if (relationed) {
			for (TableRelationDefineImpl relation : table.relations) {
		...
		if (fielded) {
			...
			for (TableFieldDefineImpl field : table.fields) {
				if (field.dbTable != table.primary || field.isRECID() || field.isRECVER()) {
					continue;
				}
		...
		if (dbTabled) {
			...
			for (DBTableDefineImpl dbTable : table.dbTables) {
				...
				builder.appendLine("%s = %s.newDBTable(\"%s\");", DBTABLE, THIS_TABLE, dbTable.getName());
		...
		if (indexed) {
			...
			for (IndexDefineImpl index : table.indexes) {
				...
				builder.append("%s%s.newIndex(\"%s\"", toSet ? INDEX + " = " : "", THIS_TABLE, index.getName());
		...
		if (hierarchied) {
			...
			for (HierarchyDefineImpl hierarchy : table.hierarchies) {
				...
				builder.appendLine("this.%s %s= %s.newHierarchy(\"%s\", %s);", declareNameOf(hierarchy), toSet ? "= " + HIERARCHY + " " : "", THIS_TABLE, hierarchy.name, hierarchy.getMaxLevel());
				if (toSet) {
		...
		if (relationed) {
			...
			for (TableRelationDefineImpl relation : table.relations) {
				if (!targets.contains(relation.target)) {
					TwoKeyEntry entry = this.get(MetaElementType.TABLE, relation.target.name);
					builder.appendLine("%1$s %2$s = querier.get(%1$s.class);", entry.clzname, declareNameOf(relation.target));
					targets.add(relation.target);
				}
				String rn = declareNameOf(relation);
				builder.appendLine("this.%s = %s.newRelation(" + "\"%s\", %s, %s.%s);", rn, THIS_TABLE, relation.name, declareNameOf(relation.target), TableRelationType.class.getSimpleName(), relation.type.toString());
	...
	private final void buildAbstractTableDeclarator(CodeBuilder builder,
		...
		builder.appendLine();
		beginClass(builder, PUBLIC | ABSTRACT, clzname, TableDeclarator.class.getSimpleName());
		builder.appendLine();
		for (NTableField field : table.primary.fields) {
			declareDefine(builder, PUBLIC | FINAL, TableFieldDefine.class, declareNameOf(field));
		}
		if (dbTabled) {
		...
		builder.appendLine();
		for (NTableField field : table.primary.fields) {
			declareStringWithAssign(builder, PUBLIC | FINAL | STATIC, declareNameOfFN(field), field.name.value);
		}
		if (dbTabled) {
			for (NTableExtend extend : table.extend) {
				for (NTableField field : extend.fields) {
		...
		builder.appendLine();
		builder.appendLine("%s %s(%s %s) {", Modifier.toString(PROTECTED), clzname, String.class.getSimpleName(), TABLE_NAME).pi();
		builder.appendLine("super(%s);", TABLE_NAME);
		if (toSetFieldAttrs) {
			declareVariable(builder, TableFieldDeclare.class, FIELD);
		}
		...
		if (dbTabled) {
			declareVariable(builder, DBTableDeclare.class, DBTABLE);
			for (NTableExtend extend : table.extend) {
				builder.appendLine("%s = %s.newDBTable(\"%s\");", DBTABLE, THIS_TABLE, extend.name.value);
				for (NTableField field : extend.fields) {
		...
		if (indexed) {
			...
			for (NTableIndex index : table.index) {
				...
				builder.append("%s%s.newIndex(\"%s\"", toSet ? INDEX + " = " : "", THIS_TABLE, index.name.value);
				...
				if (toSet) {
					...
					for (int i = 0, c = index.fields.length; i < c; i++) {
						if (index.fields[i].desc) {
							builder.appendLine("%s.getItems().get(%s)" + ".setDesc(true);", INDEX, i);
						}
					}
				}
	...
	private static final void buildQueryStatementDeclarator(
		...
		builder.appendLine();
		beginClass(builder, PUBLIC | FINAL, clzname, QueryStatementDeclarator.class);
		builder.appendLine().pi();
		if (arged) {
			for (NParamDeclare arg : query.params) {
				declareDefine(builder, PUBLIC | FINAL, ArgumentDefine.class, declareNameOf(arg));
			}
			builder.appendLine();
		}
		NQuerySpecific s = query.body.getMasterSelect();
		for (NQueryColumn column : s.select.columns) {
			declareDefine(builder, PUBLIC | FINAL, QueryColumnDefine.class, declareNameOf(column));
		}
		builder.appendLine();
		...
		if (arged) {
			for (NParamDeclare arg : query.params) {
				builder.appendLine("this.%s = this.query.getArguments().get(%s);", declareNameOf(arg), i++);
			}
		}
		i = 0;
		for (NQueryColumn column : s.select.columns) {
			builder.appendLine("this.%s = this.query.getColumns().get(%s);", declareNameOf(column), i++);
		}
	...
	private static final void buildORMDeclarator(CodeBuilder builder,
		...
		builder.appendLine();
		builder.appendLine("public class %s extends ORMDeclarator<%s> {", clzname, orm.className);
		builder.appendLine().pi();
		if (arged) {
		...
		if (arged) {
			for (NParamDeclare arg : orm.params) {
				builder.appendLine("this.%s = this.orm.getArguments().get(%s);", declareNameOf(arg), i++);
			}
		}
		i = 0;
		for (NQueryColumn column : s.select.columns) {
			builder.appendLine("this.%s = this.orm.getColumns().get(%s);", declareNameOf(column), i++);
		}
		builder.ri().appendLine('}');
		builder.ri().appendLine('}');
	}
	...
	private static final void buildModifyStatementDeclarator(
		...
		builder.appendLine();
		beginClass(builder, PUBLIC | FINAL, clzname, extendsClz);
		builder.appendLine().pi();
		if (arged) {
		...
		if (arged) {
			for (NParamDeclare arg : statement.params) {
				builder.appendLine("this.%s = this.statement.getArguments().get(%s);", declareNameOf(arg), i++);
			}
		}
		builder.ri().appendLine('}');
		builder.ri().appendLine('}');
	...
	private static final String TypeFactoryRef = TypeFactory.class.getSimpleName().concat(".");

	private static final String defineType(DataType type) {
		return TypeFactoryRef.concat(type.toString().toUpperCase());
	}
	...
	private static final boolean toSetAttrs(NamedDefine define) {
		if (notNull(define.getDescription()) || notNull(define.getTitle())) {
			...
			return f.isKeepValid() || f.getDefault() != null || notNull(f.getNameInDB()) && !f.getNameInDB().equals(f.name.toUpperCase());
	...
	private static final void beginClass(CodeBuilder builder, int modifier,
		...
		builder.append("%s class %s %s", Modifier.toString(modifier), clzname, notNull(extendsClass) ? "extends " + extendsClass : "");
		builder.appendLine(" {").appendLine().pi();
	}

	private static final void beginClass(CodeBuilder builder, int modifier,
			String clzname, Class<?> extClz) throws IOException {
		builder.append("%s class %s %s", Modifier.toString(modifier), clzname, extClz != null ? "extends " + extClz.getSimpleName() : "");
		builder.appendLine(" {").appendLine().pi();
	}
	...
	private static final void declareDefine(CodeBuilder builder, int modifier,
		...
		builder.appendLine("%s %s %s;", Modifier.toString(modifier), clz.getSimpleName(), name);
	}
	...
	private static final void declareStringWithAssign(CodeBuilder builder,
			...
			throws IOException {
		// 没有转义
		builder.appendLine("%s String %s%s;", Modifier.toString(modifier), identifier, assignValue != null ? " =\"" + assignValue + "\"" : "");
	}
	...
	private static final void setAttrs(CodeBuilder builder, NamedDefine define,
		...
		if (notNull(define.getDescription())) {
			builder.appendLine("%s.setDescription(\"%s\");", from, define.getDescription());
		}
		...
		if (define instanceof TableDefineImpl) {
			...
			if (notNull(table.getCategory())) {
				builder.appendLine("%s.setCategory(\"%s\");", THIS_TABLE, table.getCategory());
			}
		} else if (define instanceof TableFieldDefineImpl) {
			...
			if (notNull(nameInDB) && !nameInDB.equals(field.getName().toUpperCase())) {
	...
	private static final void construct(CodeBuilder builder,
			...
			throws IOException {
		builder.appendLine("this.%s %s= " + "%s.%sField(%s, %s);", declareNameOf(field), toSet ? "= " + FIELD + " " : "", from, field.isPrimaryKey() ? "newPrimary" : "new", declareNameOfFN(field), defineType(field.getType()));
	}
	...
	private static final void construct(CodeBuilder builder, NTableField field,
		...
		builder.appendLine("this.%s %s= %s.%sField(%s, %s);", declareNameOf(field), set ? "= field " : "", fromPrimary ? THIS_TABLE : DBTABLE, field.primaryKey ? "newPrimary" : "new", declareNameOfFN(field), defineType(field.type.getType(null)));
		if (set) {
			if (field.defaultValue != null) {
				ConstExpr expr = (ConstExpr) new SQLExprContext(false).build(field.defaultValue);
	...
	private static final void beginConstructor(CodeBuilder builder,
		...
		builder.appendLine("%s %s() {", Modifier.toString(modifier), clzname).pi();
	}
	...
	private static final void beginDeclareUseRef(CodeBuilder builder)
		...
		builder.appendLine("protected void declareUseRef(%s querier) {", ObjectQuerier.class.getSimpleName());
		builder.pi();
	}
