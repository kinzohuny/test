...
public class TBJdpTbTradeDao {
	...
	private static final String QUERY_LIST = "SELECT tid,STATUS,buyer_nick,created,modified,jdp_response,jdp_created,jdp_modified"
			...
			+ " where seller_nick = ? and jdp_modified >= ? and created >= ?"
			+ " order by jdp_modified"
			+ " LIMIT ?,?";
	
	public List<TBJdpTbTradeModel> queryList(String seller_nick, String lastTime, String minCreated, int start, int size) throws SQLException{
		return TBDbUtils.queryList(TBJdpTbTradeModel.class, QUERY_LIST, new Object[] {seller_nick, lastTime, minCreated, start, size});
	}
}
