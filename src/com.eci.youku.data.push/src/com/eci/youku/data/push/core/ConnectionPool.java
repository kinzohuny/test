package com.eci.youku.data.push.core;
...
public class ConnectionPool {
	...
	public synchronized void createPool() throws InstantiationException,
		...
		logger.info("create database connections pool...");
		...
		logger.info("database connections pool create success!");
	}
	...
	private void createConnections(int numConnections) throws SQLException {
		...
		for (int x = 0; x < numConnections; x++) {
			...
			try {
				...
				logger.error("database connections pool create failed!", e);
				throw e;
			}
			logger.info("database connections has been created...");
		}
	}
			...
			if (!pConn.isBusy()) {
				...
				if (!testConnection(conn)) {
					...
					try {
						...
						logger.error("database connections create failed!", e);
						return null;
					}
